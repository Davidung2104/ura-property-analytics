...
function normalizeTx(tx) {
  ...
  if (!d) return null;
  const area = parseFloat(tx.area) || 0;
  const price = parseFloat(tx.price) || 0;
  const psf = area > 0 ? Math.round(price / area) : 0;
  const fr = (tx.floorRange || '').replace(/\s/g, '');
  // Normalize floor range to bands: "01" to "05" → "01-05"
  const parts = fr.split('to');
  const floorBand = parts.length === 2 ? `${parts[0].padStart(2,'0')}-${parts[1].padStart(2,'0')}` : fr;
  const floorMid = parts.length === 2 ? (parseInt(parts[0]) + parseInt(parts[1])) / 2 : parseInt(fr) || 0;
  // Tenure category
...
export function transformAll(rawTransactions) {
  const txs = rawTransactions.map(normalizeTx).filter(Boolean).filter(t => t.psf > 0 && t.area > 0);
  ...
  const psfMin = Math.floor(Math.min(...allPsf) / 200) * 200;
  const psfMax = Math.ceil(Math.max(...allPsf) / 200) * 200;
  const sHist = [];
  ...
  const rMin = Math.floor(Math.min(...rentEsts) / 500) * 500;
  const rMax = Math.ceil(Math.max(...rentEsts) / 500) * 500;
