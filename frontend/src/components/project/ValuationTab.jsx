...
export default function ValuationTab({ projInfo, projData }) {
  ...
  const t1 = useMemo(() => {
    ...
    return { psf: pricingMode === 'latest' ? sorted[0].psf : avgP, count: useTx.length, txs: sorted, period: isRecent ? 'Last 12 months' : 'All time' };
  }, [projData, pricingMode]);
  ...
  const timeAdjust = useCallback((psf, txDate) => {
    if (!psf || !txDate || projCagr === null || projCagr === undefined) return { adjPsf: psf, months: 0, rate: 0 };
    const now = new Date();
  ...
  // Latest mode → latest tx date. Average mode → median date.
  const t1Adj = useMemo(() => timeAdjust(t1.psf, pricingMode === 'latest' ? t1.txs?.[0]?.date : medianDate(t1.txs)), [t1, timeAdjust, pricingMode]);
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      {/* ── Transaction Scatter ── */}
      <SectionHeader icon="⬡" title="Transaction Scatter: Size × Floor × PSF" sub="Most recent 80 transactions (all time). Bubble size = floor level." />
      ...
      <Card>
        ...
        <div style={{ color: T.textMute, fontSize: T.sm, marginBottom: 14, lineHeight: 1.5 }}>
          {pricingMode === 'latest'
            ? '📌 Latest: Uses the single most recent transaction PSF for each tier. Best for fast-moving markets.'
            : '📊 Average: Uses the mean PSF across all matching transactions in each tier. Smooths out outliers.'}
        </div>
        ...
        {bestRaw > 0 && <div style={{ background: 'linear-gradient(135deg,#a78bfa18,#38bdf818)', borderRadius: T.rLg, padding: '18px 20px', border: '1px solid #a78bfa26', marginBottom: 12 }}>
          ...
          {bestAdj.months > 0 && bestAdjPsf !== bestRaw && <div style={{ color: T.purple, fontSize: T.sm, marginTop: 4, display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {[
            { tier: 1, label: 'PROJECT AVG', desc: `All sizes · All floors · ${t1.period}`, psf: t1.psf, adjPsf: t1Adj.adjPsf, adj: t1Adj, cnt: t1.count, txs: t1.txs, c: T.textMute, ic: '📊', act: !t2.psf && !t3.psf && !t4.psf },
            { tier: 2, label: 'SIZE MATCH', desc: `${nearSize.toLocaleString()} sqft · Any floor`, psf: t2.psf, adjPsf: t2Adj.adjPsf, adj: t2Adj, cnt: t2.cnt, txs: t2.txs, c: T.blue, ic: '📐', act: t2.psf > 0 && !t4.psf },
            ...(pFloor ? [{ tier: 3, label: 'FLOOR MATCH', desc: `Any size · Floor ${pFloor}`, psf: t3.psf, adjPsf: t3Adj.adjPsf, adj: t3Adj, cnt: t3.cnt, txs: t3.txs, c: T.amber, ic: '🏢', act: t3.psf > 0 && !t4.psf }] : []),
            ...(pFloor && nearSize ? [{ tier: 4, label: 'EXACT MATCH', desc: `${nearSize.toLocaleString()} sqft · Floor ${pFloor}`, psf: t4.psf, adjPsf: t4Adj.adjPsf, adj: t4Adj, cnt: t4.cnt, txs: t4.txs, c: T.green, ic: '🎯', act: t4.psf > 0 }] : [])
          ].map(t => t.psf > 0 && <div key={t.tier}>
            <div onClick={() => setExpandedTier(expandedTier === t.tier ? null : t.tier)} style={{ background: t.act ? `${t.c}08` : T.borderLt, border: `1px solid ${t.act ? t.c + '4D' : T.border}`, borderRadius: expandedTier === t.tier ? `${T.rLg}px ${T.rLg}px 0 0` : T.rLg, padding: '16px 18px', position: 'relative', overflow: 'hidden', cursor: 'pointer', transition: 'all 0.15s' }}>
              ...
              {t.adj.months > 0 && t.adjPsf !== t.psf && <div style={{ color: T.purple, fontSize: T.xs, marginTop: 2 }}>⏱️ Adj today: ${t.adjPsf.toLocaleString()} PSF · ${(t.adjPsf * pArea).toLocaleString()}</div>}
