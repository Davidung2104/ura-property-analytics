...
export default function ValuationTab({ projInfo, projData }) {
  ...
  // ── Time windows: 3M and 12M independently (no all-time) ──
  const txWindows = useMemo(() => {
    if (!projData?.txs?.length) return { m3: [], m12: [] };
    const now = new Date();
    const d3 = new Date(now); d3.setMonth(d3.getMonth() - 3);
    const m3Date = `${d3.getFullYear()}-${String(d3.getMonth() + 1).padStart(2, '0')}`;
    const m12Date = `${now.getFullYear() - 1}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    return {
      m3: projData.txs.filter(t => t.date >= m3Date),
      m12: projData.txs.filter(t => t.date >= m12Date),
    };
  }, [projData]);
  ...
  const tiers = useMemo(() => {
    const t = [
      { id: 1, label: 'PROJECT AVG', desc: 'All sizes · All floors', ic: '📊', c: T.textMute,
        m3: calcTier(txWindows.m3), m12: calcTier(txWindows.m12) },
      { id: 2, label: 'SIZE MATCH', desc: `${nearSize.toLocaleString()} sqft · Any floor`, ic: '📐', c: T.blue,
        m3: calcTier(sizeFilter(txWindows.m3)), m12: calcTier(sizeFilter(txWindows.m12)) },
    ];
    if (pFloor) {
      t.push({ id: 3, label: 'FLOOR MATCH', desc: `Any size · Floor ${pFloor}`, ic: '🏢', c: T.amber,
        m3: calcTier(floorFilter(txWindows.m3)), m12: calcTier(floorFilter(txWindows.m12)) });
    }
    if (pFloor && nearSize) {
      t.push({ id: 4, label: 'EXACT MATCH', desc: `${nearSize.toLocaleString()} sqft · Floor ${pFloor}`, ic: '🎯', c: T.green,
        m3: calcTier(exactFilter(txWindows.m3)), m12: calcTier(exactFilter(txWindows.m12)) });
    }
    return t;
  ...
  // Best estimate: prefer highest specificity tier that has 3M data, else 12M
  const bestTier = useMemo(() => {
    // Try exact → size → floor → project, preferring 3M then 12M
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      {/* ── Price Estimator ── */}
      <SectionHeader icon="🧮" title="Price Estimator" sub="Average PSF by tier. 3M = recent market pulse, 12M = fair value benchmark." />
      <Card>
        ...
        {bestRaw > 0 && <div style={{ background: 'linear-gradient(135deg,#a78bfa18,#38bdf818)', borderRadius: T.rLg, padding: '18px 20px', border: '1px solid #a78bfa26', marginBottom: 16 }}>
          <div style={{ color: T.textMute, fontSize: T.sm, fontWeight: 600, letterSpacing: 0.5, marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span title="Uses most specific tier with data. Prefers 3M, falls back to 12M." style={{ cursor: 'help', opacity: 0.6, fontSize: T.base }}>ℹ️</span>
          </div>
        ...
        {/* Column headers */}
        <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr 1fr', gap: 8, marginBottom: 6, padding: '0 4px' }}>
          <div />
        ...
        {/* Tier rows */}
        {tiers.map(t => {
          const m3 = t.m3, m12 = t.m12;
          const hasAny = m3.psf > 0 || m12.psf > 0;
          if (!hasAny) return null;
          const exp3m = expandedTier === t.id + '-3m';
          const exp12m = expandedTier === t.id + '-12m';
          const expandTxs = exp3m ? m3.txs : exp12m ? m12.txs : null;
          const expandLabel = exp3m ? '3M' : '12M';
          return <div key={t.id} style={{ marginBottom: 8 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr 1fr', gap: 8, alignItems: 'stretch' }}>
              {/* Tier label */}
              ...
              {m3.psf > 0 ? <div onClick={() => setExpandedTier(exp3m ? null : t.id + '-3m')}
                ...
                <span style={{ color: T.textMute, fontSize: T.sm }}>No transactions</span>
              </div>}
              {/* 12M cell */}
            ...
            {/* Expanded tx table */}
            {(exp3m || exp12m) && expandTxs?.length > 0 && <div style={{ background: T.borderLt, border: `1px solid ${T.border}`, borderTop: 'none', borderRadius: `0 0 ${T.rLg}px ${T.rLg}px`, padding: '12px 14px', maxHeight: 280, overflowY: 'auto', marginTop: -1 }}>
              ...
              <table style={{ margin: 0 }}>
                ...
                <tbody>
                  {expandTxs.slice(0, 30).map((tx, i) => <tr key={i}>
                  ...
                  {expandTxs.length > 30 && <tr><td colSpan={6} style={{ color: T.textMute, fontSize: T.sm, textAlign: 'center', padding: 8 }}>Showing 30 of {expandTxs.length}</td></tr>}
                </tbody>
              </table>
            </div>}
          </div>;
