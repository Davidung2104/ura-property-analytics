...
export default function ValuationTab({ projInfo, projData }) {
  ...
  const bedOptions = projData?.bedOptions || [];

  // Filtered transactions (by bedroom)
  const filteredTxs = useMemo(() => {
    const txs = projData?.txs || [];
    if (bedsFilter === 'all') return txs;
    return txs.filter(t => t.beds === bedsFilter);
  }, [projData, bedsFilter]);

  // Recompute heatmap matrix from filtered txs
  const filteredHm = useMemo(() => {
    const hmYears = projData?.hmYears || [];
    const hmFloors = projData?.hmFloors || [];
    if (bedsFilter === 'all') return { matrix: projData?.hmMatrix || {}, years: hmYears, floors: hmFloors };
    const matrix = {};
    hmFloors.forEach(f => {
  ...
  }, [projData, filteredTxs, bedsFilter]);
  ...
  // Heatmap matrix — bedroom-filtered base, recompute when size filter active
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      {bedOptions.length >= 2 && (
        <div style={{ position: 'sticky', top: 0, zIndex: 20, margin: '0 -16px', padding: '0 16px' }}>
        <Card>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ fontSize: T.base }}>🛏️</span>
              <span style={{ color: T.textMute, fontSize: T.sm, fontWeight: 600, letterSpacing: 0.5 }}>BEDROOM FILTER</span>
            </div>
            <div style={{ display: 'flex', gap: 6 }}>
              <button onClick={() => setBedsFilter('all')} style={{
                background: bedsFilter === 'all' ? T.purple : T.borderLt, color: bedsFilter === 'all' ? '#fff' : T.textSub,
                border: `1px solid ${bedsFilter === 'all' ? T.purple : T.border}`, borderRadius: 6, padding: '4px 14px',
                fontSize: T.sm, cursor: 'pointer', fontWeight: 600,
              }}>All</button>
              {bedOptions.map(b => (
                <button key={b} onClick={() => setBedsFilter(b)} style={{
                  background: bedsFilter === b ? T.purple : T.borderLt, color: bedsFilter === b ? '#fff' : T.textSub,
                  border: `1px solid ${bedsFilter === b ? T.purple : T.border}`, borderRadius: 6, padding: '4px 14px',
                  fontSize: T.sm, cursor: 'pointer', fontWeight: 600, fontFamily: T.mono,
                }}>{b} BR</button>
              ))}
            </div>
            {bedsFilter !== 'all' && (
              <span style={{ color: T.purple, fontSize: T.sm, fontWeight: 600 }}>
                {filteredTxs.length} of {(projData?.txs || []).length} transactions
              </span>
            )}
            <span style={{ color: T.textMute, fontSize: T.xs, fontStyle: 'italic' }}>Inferred from URA rental area↔bedroom data</span>
          </div>
        </Card>
        </div>
      )}
