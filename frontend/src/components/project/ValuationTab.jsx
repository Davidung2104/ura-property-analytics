...
export default function ValuationTab({ projInfo, projData }) {
  ...
  // Tier 1: Project average (last 12 months, fallback to all-time if < 3 recent)
  const t1 = useMemo(() => {
    if (!projData?.txs?.length) return { psf: 0, count: 0, txs: [], period: '' };
    const now = new Date();
    const m12 = `${now.getFullYear() - 1}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    const recentTx = projData.txs.filter(t => t.date >= m12);
    const useTx = recentTx.length >= 3 ? recentTx : projData.txs;
    const isRecent = recentTx.length >= 3;
    const sorted = [...useTx].sort((a, b) => b.date.localeCompare(a.date));
    const avgP = Math.round(useTx.reduce((s, t) => s + t.psf, 0) / useTx.length);
    return { psf: avgP, count: useTx.length, txs: sorted, period: isRecent ? 'Last 12 months' : 'All time' };
  }, [projData]);

  // Tier 2: Size match
  const nearSize = projSizes.length > 0 ? projSizes.reduce((prev, c) => Math.abs(c - pArea) < Math.abs(prev - pArea) ? c : prev, projSizes[0]) : pArea;
  const t2 = useMemo(() => {
    if (!projData?.txs?.length) return { psf: 0, cnt: 0, lp: 0, txs: [] };
    const sizeTx = projData.txs.filter(t => Math.abs(t.area - nearSize) < 50);
    if (!sizeTx.length) return { psf: 0, cnt: 0, lp: 0, txs: [] };
    const sorted = [...sizeTx].sort((a, b) => b.date.localeCompare(a.date));
    const avgP = Math.round(sizeTx.reduce((s, t) => s + t.psf, 0) / sizeTx.length);
    return { psf: pricingMode === 'latest' ? sorted[0].psf : avgP, cnt: sizeTx.length, lp: sorted[0].psf, txs: sorted };
  }, [projData, nearSize, pricingMode]);

  // Tier 3: Floor match
  const t3 = useMemo(() => {
    if (!pFloor || !projData?.txs?.length) return { psf: 0, cnt: 0, txs: [] };
    const [lo, hi] = pFloor.split('-').map(Number);
    const floorTx = projData.txs.filter(t => t.floorMid >= lo && t.floorMid <= hi);
    if (!floorTx.length) return { psf: 0, cnt: 0, txs: [] };
    const sorted = [...floorTx].sort((a, b) => b.date.localeCompare(a.date));
    const avgP = Math.round(floorTx.reduce((s, t) => s + t.psf, 0) / floorTx.length);
    return { psf: pricingMode === 'latest' ? sorted[0].psf : avgP, cnt: floorTx.length, txs: sorted };
  }, [projData, pFloor, pricingMode]);
  ...
  const t4 = useMemo(() => {
    ...
    if (!exactTx.length) return { psf: 0, cnt: 0, txs: [] };
    const sorted = [...exactTx].sort((a, b) => b.date.localeCompare(a.date));
    const avgP = Math.round(exactTx.reduce((s, t) => s + t.psf, 0) / exactTx.length);
    return { psf: pricingMode === 'latest' ? sorted[0].psf : avgP, cnt: exactTx.length, txs: sorted };
  }, [projData, nearSize, pFloor, pricingMode]);

  // ── Project CAGR for time adjustment ──
  const projCagr = useMemo(() => {
    const r = computeBucketCAGR(projData?.rawTx || []);
    return r.cagr !== null ? r.cagr : 0;
  }, [projData]);

  // Time-adjust a PSF value from a transaction date to today using pro-rated CAGR
  const timeAdjust = useCallback((psf, txDate) => {
    if (!psf || !txDate || !projCagr) return { adjPsf: psf, months: 0, rate: 0 };
    const now = new Date();
  ...
  // Time-adjusted PSF for each tier
  // T1 always uses median (it's always an average). T2-T4: latest mode → latest tx date, average mode → median date.
  const t1Adj = useMemo(() => timeAdjust(t1.psf, medianDate(t1.txs)), [t1, timeAdjust]);
  const t2Adj = useMemo(() => timeAdjust(t2.psf, pricingMode === 'latest' ? t2.txs?.[0]?.date : medianDate(t2.txs)), [t2, timeAdjust, pricingMode]);
  const t3Adj = useMemo(() => timeAdjust(t3.psf, pricingMode === 'latest' ? t3.txs?.[0]?.date : medianDate(t3.txs)), [t3, timeAdjust, pricingMode]);
  const t4Adj = useMemo(() => timeAdjust(t4.psf, pricingMode === 'latest' ? t4.txs?.[0]?.date : medianDate(t4.txs)), [t4, timeAdjust, pricingMode]);

  const bestAdj = t4Adj.adjPsf && t4.psf ? t4Adj : t2Adj.adjPsf && t2.psf ? t2Adj : t3Adj.adjPsf && t3.psf ? t3Adj : t1Adj;
  const bestPsf = bestAdj.adjPsf || t4.psf || t2.psf || t3.psf || t1.psf;
  const bestRaw = t4.psf || t2.psf || t3.psf || t1.psf;
    ...
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <Card>
        ...
        {bestPsf > 0 && <div style={{ background: 'linear-gradient(135deg,#a78bfa18,#38bdf818)', borderRadius: T.rLg, padding: '18px 20px', border: '1px solid #a78bfa26', marginBottom: 12 }}>
          <div style={{ color: T.textMute, fontSize: T.sm, fontWeight: 600, letterSpacing: 0.5, marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>BEST ESTIMATE · {pricingMode === 'latest' ? 'LATEST TX' : 'SMART AVERAGE'}<span title="Cascading match: exact (size+floor) → size → floor → project avg. Time-adjusted using project CAGR." style={{ cursor: 'help', opacity: 0.6, fontSize: T.base }}>ℹ️</span></div>
          ...
          {bestAdj.months > 0 && <div style={{ color: T.purple, fontSize: T.sm, marginTop: 4, display: 'flex', alignItems: 'center', gap: 4 }}>
            <span>⏱️</span> Time-adjusted +{bestAdj.months}mo at {bestAdj.rate > 0 ? '+' : ''}{Number(bestAdj.rate).toFixed(1)}% CAGR
            {bestRaw !== bestPsf && <span style={{ color: T.textMute }}> · raw ${bestRaw.toLocaleString()} PSF</span>}
          </div>}
        </div>}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {[
            { tier: 1, label: 'PROJECT AVG', desc: `All sizes · All floors · ${t1.period}`, psf: t1Adj.adjPsf || t1.psf, rawPsf: t1.psf, adj: t1Adj, cnt: t1.count, txs: t1.txs, c: T.textMute, ic: '📊', act: !t2.psf && !t3.psf && !t4.psf },
            { tier: 2, label: 'SIZE MATCH', desc: `${nearSize.toLocaleString()} sqft · Any floor`, psf: t2Adj.adjPsf || t2.psf, rawPsf: t2.psf, adj: t2Adj, cnt: t2.cnt, txs: t2.txs, c: T.blue, ic: '📐', act: t2.psf > 0 && !t4.psf },
            ...(pFloor ? [{ tier: 3, label: 'FLOOR MATCH', desc: `Any size · Floor ${pFloor}`, psf: t3Adj.adjPsf || t3.psf, rawPsf: t3.psf, adj: t3Adj, cnt: t3.cnt, txs: t3.txs, c: T.amber, ic: '🏢', act: t3.psf > 0 && !t4.psf }] : []),
            ...(pFloor && nearSize ? [{ tier: 4, label: 'EXACT MATCH', desc: `${nearSize.toLocaleString()} sqft · Floor ${pFloor}`, psf: t4Adj.adjPsf || t4.psf, rawPsf: t4.psf, adj: t4Adj, cnt: t4.cnt, txs: t4.txs, c: T.green, ic: '🎯', act: t4.psf > 0 }] : [])
          ].map(t => t.rawPsf > 0 && <div key={t.tier}>
            <div onClick={() => setExpandedTier(expandedTier === t.tier ? null : t.tier)} style={{ background: t.act ? `${t.c}08` : T.borderLt, border: `1px solid ${t.act ? t.c + '4D' : T.border}`, borderRadius: expandedTier === t.tier ? `${T.rLg}px ${T.rLg}px 0 0` : T.rLg, padding: '16px 18px', position: 'relative', overflow: 'hidden', cursor: 'pointer', transition: 'all 0.15s' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ color: T.textMute, fontSize: T.sm, transition: 'transform 0.2s', display: 'inline-block', transform: expandedTier === t.tier ? 'rotate(180deg)' : 'rotate(0deg)' }}>▾</span>
                </div>
              </div>
              ...
              {t.adj.months > 0 && <div style={{ color: T.purple, fontSize: T.xs, marginTop: 2 }}>⏱️ +{t.adj.months}mo adj · raw ${t.rawPsf.toLocaleString()} PSF</div>}
              <div style={{ color: T.textSub, fontSize: T.xs, marginTop: 2 }}>{t.desc}</div>
            </div>
            {expandedTier === t.tier && <div style={{ background: T.borderLt, border: `1px solid ${t.act ? t.c + '4D' : T.border}`, borderTop: 'none', borderRadius: `0 0 ${T.rLg}px ${T.rLg}px`, padding: '12px 14px', maxHeight: 280, overflowY: 'auto' }}>
              <div style={{ color: T.textSub, fontSize: T.sm, fontWeight: 600, marginBottom: 8 }}>{t.label} — {t.cnt} transactions</div>
              <table style={{ margin: 0 }}>
                <thead><tr>
                  {['Date', 'Floor', 'Area', 'Price', 'PSF', 'Type'].map(h => <th key={h} style={{ fontSize: T.sm, padding: '5px 8px' }}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {(t.txs || []).slice(0, 30).map((tx, i) => <tr key={i}>
                  ...
                  {(t.txs || []).length > 30 && <tr><td colSpan={6} style={{ color: T.textMute, fontSize: T.sm, textAlign: 'center', padding: 8 }}>Showing 30 of {t.txs.length}</td></tr>}
                </tbody>
              </table>
            </div>}
          </div>)}
      ...
      {/* ── VALUATION MODEL ── */}
      {valuationModel && <><SectionHeader icon="🔍" title="Valuation Model" sub={`Comparable Market Analysis — ${valuationModel.totalTx} transactions scored by recency, size & floor similarity, time-adjusted at ${projCagr > 0 ? '+' : ''}${Number(projCagr).toFixed(1)}% CAGR.`} />
      <Card>
        ...
        <NoteText>Model uses weighted Comparable Market Analysis. Each transaction scored by: recency (exponential decay, ~25mo half-life), size similarity (±150sf Gaussian), floor proximity (±8 floors Gaussian). PSF adjusted forward to today using {projCagr > 0 ? '+' : ''}{Number(projCagr).toFixed(1)}% project CAGR{pFloor ? ' and floor premium differential' : ''}. Confidence based on recent comparable volume and size match density.</NoteText>
      </Card></>}
