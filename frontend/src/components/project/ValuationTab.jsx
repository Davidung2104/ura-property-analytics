...
import { T, computeBucketCAGR, cagrColor, yieldColor, getBed, DEFAULT_YIELD } from '../../constants';
          ...
          return <div>
            ...
            <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: `repeat(${Math.min(rows.length, investorMode === 'overall' ? 1 : 5)},1fr)`, gap: 8 }}>
              {rows.slice(0, investorMode === 'overall' ? 1 : 10).map((r, ri) => <div key={ri} style={{ background: T.borderLt, borderRadius: T.r, padding: '10px 12px', opacity: r.lowConf ? 0.5 : 1 }}>
                ...
                {r.cagr !== null && <div style={{ textAlign: 'center', marginTop: 4, fontSize: T.md, fontWeight: 700, color: r.cagr >= 5 ? T.green : r.cagr >= 0 ? T.lime : T.red, fontFamily: T.mono }}>CAGR {r.cagr >= 0 ? '+' : ''}{r.cagr.toFixed(1)}%</div>}
              </div>)}
            </div>
            ...
            {(() => {
              ...
              return (projData?.hmFloors || []).map(f => {
                ...
                return <div key={f} style={{ display: 'flex', borderBottom: '1px solid #f1f5f9' }} onMouseEnter={e => e.currentTarget.style.background = T.borderLt} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                  ...
                  <div style={{ width: 60, flexShrink: 0, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: T.sm, fontFamily: T.mono, fontWeight: 600, color: Number(pctChg) > 4 ? T.lime : Number(pctChg) > 2 ? T.amber : Number(pctChg) < 0 ? T.negLt : T.textMute, background: T.bg }}>{Number(pctChg) > 0 ? '+' : ''}{pctChg}%</div>
                  ...
                  <div style={{ width: 60, flexShrink: 0, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: T.sm, fontFamily: T.mono, fontWeight: 700, color: totalPct > 18 ? T.green : totalPct > 12 ? T.amber : totalPct < 0 ? T.red : T.textMute, background: T.bg }}>{totalPct > 0 ? '+' : ''}{totalPct}%</div>
                </div>;
              });
            })()}
