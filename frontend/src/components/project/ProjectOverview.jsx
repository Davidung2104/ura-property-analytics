...
export default function ProjectOverview({ projInfo, projData, masterFilters = {} }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <SectionHeader icon="📈" title="PSF Trend" sub={`Quarterly average PSF trend.${hasFilters ? ' Filtered data.' : ''}`} />
      ...
      <SectionHeader icon="💵" title="Rental Trend" sub={p.hasRealRental ? 'Average & median monthly rent from real URA rental contracts.' : 'No URA rental data available for this project.'} />
      {rentTrend.length > 0 ? (
      ...
      <SectionHeader icon="⬡" title="Transaction Scatter: Size × Floor × PSF" sub={`${scatter.length} transactions${hasFilters ? ' (filtered)' : ' (most recent 80)'}. Bubble size = floor level.`} />
