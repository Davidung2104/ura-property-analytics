...
export default function ClientReport({ projInfo, projData, clientReports, setClientReports }) {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>Client Report — ${viewingReport?.clientName || clientName || p.name}</title>
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        th { text-align: left; padding: 6px 8px; border-bottom: 2px solid #e2e8f0; color: #64748b; font-weight: 600; font-size: 11px; text-transform: uppercase; }
    ...
    return <div style={{ display: 'grid', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={handlePrint} style={btnStyle(T.blue)}>🖨️ Print / PDF</button>
          <button onClick={() => handleDelete(viewingReport.id)} style={btnStyle(T.red)}>🗑️ Delete</button>
        </div>
      </div>
      <Card>
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <SectionHeader icon="📄" title="Client Report Builder" sub="Curate pricing data and save a snapshot for your client. Saved reports freeze the data at the time of saving." />
      ...
      <Card>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(170px, 1fr))', gap: 8 }}>
          {[
            ...
            { key: 'floorPremium', label: '🏗️ Floor Premium', desc: 'Floor analysis' },
          ].map(s => (
      ...
      {/* Live Preview */}
      <SectionHeader icon="👁️" title="Live Preview" sub="This is how the report will look when saved and printed." />
      <Card>
        <div ref={printRef} style={{ padding: '8px 4px' }}>
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={handlePrint} style={{ ...btnStyle(T.blue), padding: '10px 20px' }}>🖨️ Print / PDF</button>
