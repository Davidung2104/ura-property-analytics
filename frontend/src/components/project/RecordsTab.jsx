...
export default function RecordsTab({ projInfo, projData, masterFilters = {} }) {
  ...
  // Sync rental bed filter with master beds filter
  useEffect(() => {
    if (bedsFilter !== 'all') setRentBedFilter(bedsFilter);
    else setRentBedFilter('');
  }, [bedsFilter]);
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <SectionHeader title="Sale Transactions" sub={`${filteredTx.length} of ${(projData?.projTx || []).length} sales for ${p.name}.`} />
      <Card>
      ...
      <SectionHeader title="Rental Contracts" sub={p.hasRealRental ? `${filteredRentTx.length} of ${(projData?.projRentTx || []).length} URA rental records. Aggregated by quarter.` : 'No URA rental data available for this project.'} />
      <Card>
        ...
        {(projData?.projRentTx || []).length > 0 && <div style={{ marginBottom: 12, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {(projData?.rentalBedrooms || []).length > 0 && bedsFilter === 'all' && <select value={rentBedFilter} onChange={e => setRentBedFilter(e.target.value)} style={{ background: '#fff', border: '1px solid #cbd5e1', borderRadius: T.r, padding: '6px 10px', color: T.text, fontSize: T.base, cursor: 'pointer', outline: 'none', fontFamily: T.mono }}>
            <option value="">All Bedrooms</option>
            {(projData?.rentalBedrooms || []).map(b => <option key={b} value={b}>{b} Bed</option>)}
          </select>}
          {bedsFilter !== 'all' && <span style={{ color: T.purple, fontSize: T.sm, fontWeight: 600 }}>{bedsFilter} BR (from filter bar)</span>}
