    ...
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <Card>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table>
            <thead><tr>{['Date', 'Floor', 'Area (sf)', 'Price', 'PSF', 'Type'].map(h => <th key={h}>{h}</th>)}</tr></thead>
      ...
      <SectionHeader icon="🏠" title="Rental Contracts" sub={`${(projData?.projRentTx || []).length} rental records. URA rental data is aggregated by quarter.`} />
      <Card>
        <div style={{ overflowX: 'auto' }}>
          <table>
            <thead><tr>{['Period', 'Area (sf)', 'Rent/mo', 'Rent PSF', 'Lease Date', 'Contracts'].map(h => <th key={h}>{h}</th>)}</tr></thead>
