...
export default function CompareTab({ proj, cmpPool, cmpSelected, setCmpSelected, mktData, nearbyProjects = [], selfYearPsf = {}, selfBedYearPsf = {}, bedOptions = [], projList = [], projIndex = {} }) {
  const [bedsFilter, setBedsFilter] = useState('all');

  // Collect all available bedroom types across nearby projects + self
  const allBedOptions = useMemo(() => {
    const beds = new Set(bedOptions);
    nearbyProjects.forEach(p => {
      if (p.bedYearPsf) Object.keys(p.bedYearPsf).forEach(b => beds.add(b));
    });
    return [...beds].sort((a, b) => parseInt(a) - parseInt(b));
  }, [bedOptions, nearbyProjects]);
  ...
  const allProjects = useMemo(() => {
    ...
    // Self: ensure selected project always has yearPsf + bedYearPsf
    if (map[proj]) {
      map[proj].rel = 'self';
  ...
  // Helper: get yearPsf for a project respecting bedroom filter
  const getYearPsf = useCallback((p) => {
    if (bedsFilter !== 'all' && p?.bedYearPsf?.[bedsFilter]) return p.bedYearPsf[bedsFilter];
    return p?.yearPsf || {};
  }, [bedsFilter]);

  // Helper: get latest PSF for a project respecting bedroom filter
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      {/* ── Bedroom Filter (sticky) ── */}
      {allBedOptions.length >= 2 && <div style={{ position: 'sticky', top: 0, zIndex: 20, background: T.pageBg || '#f8fafc', padding: '8px 0', margin: '0 -16px', paddingLeft: 16, paddingRight: 16 }}>
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', background: '#fff', border: `1px solid ${T.border}`, borderRadius: T.r, padding: '10px 16px' }}>
        <span style={{ color: T.textSub, fontSize: T.md, fontWeight: 600 }}>🛏️ Filter by Bedrooms:</span>
        {['all', ...allBedOptions].map(b => (
          <button key={b} onClick={() => setBedsFilter(b)} style={{
            padding: '5px 12px', borderRadius: T.r, fontSize: T.md, fontWeight: bedsFilter === b ? 700 : 500, cursor: 'pointer',
            background: bedsFilter === b ? T.purple : T.bg, color: bedsFilter === b ? '#fff' : T.textSub,
            border: `1px solid ${bedsFilter === b ? T.purple : T.border}`, transition: 'all 0.15s',
          }}>{b === 'all' ? 'All' : `${b}BR`}</button>
        ))}
        {bedsFilter !== 'all' && <span style={{ color: T.textMute, fontSize: T.sm, fontStyle: 'italic', marginLeft: 8 }}>Showing {bedsFilter}BR PSF only. Inferred from URA rental area↔bedroom data.</span>}
        </div>
      </div>}
      ...
      {/* ── Nearby PSF Heatmap ── */}
      <SectionHeader icon="🏘️" title="Nearby Project PSF Comparison" sub={`Project × Year PSF matrix. ${bedsFilter !== 'all' ? `Filtered to ${bedsFilter}BR units. ` : ''}Darker = higher PSF. Based on URA street data.`} />
      <Card>
        <div style={{ overflowX: 'auto' }}>
