...
export default function CompareTab({ proj, cmpPool, cmpSelected, setCmpSelected, mktData, nearbyProjects = [], selfYearPsf = {}, selfBedYearPsf = {}, selfYearPrice = {}, selfBedYearPrice = {}, bedOptions = [], masterFilters = {}, projList = [], projIndex = {} }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <SectionHeader title="Comparative Market Analysis" sub={`Select up to 8 projects to compare. Type to search across all ${projList.length.toLocaleString()} projects.`} />
      <Card>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {cmpSelected.map((name, i) => {
            ...
            const relLabel = rel === 'street' ? 'Same St' : rel === 'district' ? 'Same Dist' : '';
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 10 }}>
          ...
          }} style={presetBtn}>Same District</button>
          <button onClick={() => {
          ...
          }} style={presetBtn}>Same Segment</button>
        </div>
      </Card>
      ...
      <SectionHeader title="Nearby Project PSF Comparison" sub={`Project × Year PSF matrix. ${bedsFilter !== 'all' ? `Filtered to ${bedsFilter}BR units. ` : ''}Based on URA street data.`} />
      <Card>
        {/* Heatmap controls */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', marginBottom: 12 }}>
          ...
          <span style={{ fontSize: 12, color: T.textSub }}>
            From {effectiveFrom} → {effectiveTo} ({visibleYears.length}yr){currentYear > effectiveTo ? ` · ${currentYear} excluded (partial)` : ''}
          </span>
        </div>
      ...
      {sel.length >= 2 && <Card>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table><thead><tr>{['Project', 'District', 'Street', 'Txns', 'PSF', 'Yield'].map(h => <th key={h}>{h}</th>)}</tr></thead>
            <tbody>{sel.map((r, i) => {
              ...
              const relBadge = rel === 'street' ? '●' : rel === 'district' ? '○' : '';
        ...
        <NoteText style={{ marginTop: 12 }}>● Same street · ○ Same district. Projects matched using URA transaction street data.</NoteText>
      </Card>}

      {/* ══════ CHARTS ══════ */}
      {sel.length >= 2 && <>
        <SectionHeader title={`PSF Comparison${bedsFilter !== 'all' ? ` (${bedsFilter}BR)` : ''}`} sub={`${sel[0].name} at $${sel[0].psf.toLocaleString()} PSF — ${sel[0].psf < sel[1].psf ? 'cheaper' : 'more expensive'} than ${sel[1].name}.`} />
...
function ProjectSearchAdd({ projList, projIndex, nearbyProjects, cmpPool, exclude, onSelect }) {
  ...
  const renderItem = (name) => {
    ...
    const relLabel = nearbyP?.rel === 'street' ? 'St' : nearbyP?.rel === 'district' ? 'Dist' : '';
    return (
      <div key={name}
