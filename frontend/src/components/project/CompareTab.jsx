        ...
        <div style={{ overflowX: 'auto' }}>
          <div style={{ minWidth: 600 }}>
            ...
            <div style={{ display: 'flex', borderBottom: '2px solid #1e293b' }}>
              <div style={{ width: 160, flexShrink: 0, fontSize: 13, fontWeight: 700, color: '#1e293b', padding: '12px 8px', letterSpacing: 0.3 }}>Project Name</div>
              {visibleYears.map(y => {
                const isPartial = y === currentYear;
                return <div key={y} style={{ flex: 1, minWidth: 76, textAlign: 'center', fontSize: 13, fontWeight: 700, color: '#1e293b', padding: '12px 4px' }}>{y}{isPartial ? <span style={{ fontSize: 10, color: T.orange, fontWeight: 500 }}> *</span> : ''}</div>;
              })}
              <div style={{ width: 88, flexShrink: 0, textAlign: 'center', fontSize: 13, fontWeight: 700, color: '#1e293b', padding: '12px 4px', background: '#f8fafc', borderLeft: '1px solid #e2e8f0' }}>Growth</div>
              <div style={{ width: 88, flexShrink: 0, textAlign: 'center', fontSize: 13, fontWeight: 700, color: '#1e293b', padding: '12px 4px', background: '#f8fafc', borderLeft: '1px solid #f1f5f9' }}>CAGR</div>
            </div>
            {/* Data rows */}
            {nearbyHm.projects.map((pName, rowIdx) => {
              ...
              return <div key={pName} style={{ display: 'flex', borderBottom: '1px solid #e2e8f0', background: rowBg, transition: 'background 0.15s' }} onMouseEnter={e => { e.currentTarget.style.background = isSelf ? '#ede9fe' : '#f1f5f9'; }} onMouseLeave={e => { e.currentTarget.style.background = rowBg; }}>
                <div style={{ width: 160, flexShrink: 0, fontSize: 13, fontWeight: isSelf ? 700 : 500, color: isSelf ? '#7c3aed' : '#1e293b', padding: '14px 8px', display: 'flex', alignItems: 'center', gap: 6 }}>
                  {isSelf && <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#7c3aed', flexShrink: 0 }} />}{pName}
                </div>
                {visibleYears.map(y => {
                  const psf = nearbyHm.data[`${pName}-${y}`];
                  if (!psf) return <div key={y} style={{ flex: 1, minWidth: 76, height: 48, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8', fontSize: 13 }}>—</div>;
                  ...
                  const bg = hmGradient
                    ...
                    : 'transparent';
                  const textColor = hmGradient && ratio > 0.45 ? '#fff' : '#1e293b';
                ...
                <div style={{ width: 88, flexShrink: 0, height: 48, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: T.mono, background: '#f8fafc', borderLeft: '1px solid #e2e8f0' }}>
                  {hasGrowthData ? <>
                    ...
                    <span style={{ fontSize: 10, color: '#94a3b8', fontWeight: 400 }}>{actualFromYear}→{actualToYear.slice(2)}</span>
                ...
                <div style={{ width: 88, flexShrink: 0, height: 48, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: T.mono, background: '#f8fafc', borderLeft: '1px solid #f1f5f9' }}>
                  {hasGrowthData ? <>
                    ...
                    <span style={{ fontSize: 10, color: '#94a3b8', fontWeight: 400 }}>{actualSpan}yr p.a.</span>
                  </> : <span style={{ color: '#94a3b8', fontSize: 13 }}>—</span>}
                </div>
              </div>;
            })}
            {/* Legend */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, marginTop: 14, fontSize: 12, color: '#64748b' }}>
