...
export default function ValuationModel({ valuationModel, pArea, pFloor, projCagr }) {
  ...
  return <>
    <SectionHeader icon="🔍" title="Valuation Model" sub={`Comparable Market Analysis — ${vm.totalTx} transactions scored by recency, size & floor similarity${projCagr != null ? `, time-adjusted at ${projCagr > 0 ? '+' : ''}${Number(projCagr).toFixed(1)}% CAGR` : ''}.`} />
    <Card>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${vm.floorMatches != null ? 5 : 4}, 1fr)`, gap: 10, marginBottom: 16 }}>
        {[
          ...
          { label: 'Last 12 Months', val: vm.recent12mo, ic: '🗓️' },
