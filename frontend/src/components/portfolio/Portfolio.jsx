    ...
    <Card style={{ border: `2px solid ${T.green}40` }}>
      ...
      {fProj && fSize && fPsf && (() => {
        ...
        return <div style={{ background: T.borderLt, borderRadius: T.r, padding: 12, marginBottom: 16, display: 'flex', gap: 24, flexWrap: 'wrap', fontSize: T.base }}>
          ...
          {!m && <span style={{ color: T.amber }}>⚠️ Limited market data — project may have fewer transactions</span>}
        </div>;
      })()}
    ...
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      {enriched.map(h => {
        ...
        return <Card key={h.id}>
          ...
          {!h.found && <div style={{ background: `${T.amber}15`, border: `1px solid ${T.amber}40`, borderRadius: T.r, padding: '8px 14px', marginBottom: 12, color: T.amber, fontSize: T.md }}>⚠️ This project has insufficient URA transaction data (fewer than 3 recorded transactions). Current market valuation unavailable.</div>}
