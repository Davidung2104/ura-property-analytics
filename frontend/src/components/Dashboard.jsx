...
export default function Dashboard() {
  ...
  const projFilterOpts = useMemo(() => {
    ...
    return {
      districts: [...new Set(entries.map(p => p.dist))].sort((a, b) => parseInt(a) - parseInt(b)),
  ...
  const p = projData?.projInfo || { name: 'Loading...', district: '', segment: '', tenure: '', type: '', units: 0, avgPsf: 0, psfPeriod: '', medPsf: 0, totalTx: 0, avgRent: 0, rentPsf: 0, yield: 0, distAvg: 0, hasRealRental: false };
  ...
  const projProps = {
    ...
    compare: { proj, cmpPool, cmpSelected, setCmpSelected, mktData, nearbyProjects: projData?.nearbyProjects || [] },
  ...
  return (
    <Shell>
        ...
        {sec === 'analyze' && <>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10, alignItems: 'center' }}>
            <select value={projFilters.district} onChange={e => setProjFilters(f => ({ ...f, district: e.target.value }))} style={S.filterSel(projFilters.district !== 'all')}>
              ...
              {projFilterOpts.districts.map(d => <option key={d} value={d}>D{d}</option>)}
            </select>
      ...
      {sec === 'market' && mktData && <div style={{ padding: '16px 28px' }}><div className="s">
        ...
        <StatCard label={`Avg PSF (${mktData.psfPeriod || mktData.latestYear || '—'})`} value={`$${mktData.avgPsf?.toLocaleString() || '—'}`} color={T.orange} icon="📐" sub={`Median $${mktData.medPsf?.toLocaleString() || '—'}`} />
      ...
      {sec === 'analyze' && <div style={{ padding: '16px 28px' }}><div className="s">
        ...
        <StatCard label="Avg Rent" value={`$${p.avgRent.toLocaleString()}/mo`} color={T.teal} icon="💵" sub={p.hasRealRental ? 'URA rental data' : `Est. from ${p.segment} yield (4Q)`} />
