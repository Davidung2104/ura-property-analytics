...
import { transformAll, transformProject, buildComparisonPool } from "../utils/transform";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (cancelled) return;
        if (raw.errors?.length > 0) console.warn('API warnings:', raw.errors);

        const txs = raw.transactions || [];
        if (txs.length === 0) { setError('No transaction data received from URA API'); setLoading(false); return; }

        const transformed = transformAll(txs);
        if (!transformed) { setError('Failed to transform transaction data'); setLoading(false); return; }
        setMktData(transformed);

        // Build project list and comparison pool
        const pool = buildComparisonPool(transformed.txs);
        setCmpPool(pool);
        setProjList(pool.map(p => p.name));

        // Set initial project (most traded)
        if (pool.length > 0) {
          const firstProj = pool[0].name;
          setProj(firstProj);
          const pd = transformProject(transformed.txs, firstProj);
          setProjData(pd);
          // Initialize comparison with same-district projects
          const thisDist = pool[0].dist;
          const sameDist = pool.filter(p => p.dist === thisDist && p.name !== firstProj).slice(0, 3).map(p => p.name);
          const nearby = pool.filter(p => p.dist !== thisDist).slice(0, 2).map(p => p.name);
  ...
  // When project changes, recompute project-level data
  const handleProjChange = useCallback((newProj) => {
    setProj(newProj);
    if (mktData?.txs) {
      const pd = transformProject(mktData.txs, newProj);
      setProjData(pd);
    }
  }, [mktData]);
  ...
  // Nearby project heatmap: compare PSF across selected projects over years
  const nearbyHm = useMemo(() => {
    if (!mktData?.txs || !cmpSelected.length) return { projects:[], years:[], data:{}, vol:{} };
    const years = mktData.years?.slice(-5) || [];
    const data = {};
    const vol = {};
    cmpSelected.forEach(name => {
      const pTx = mktData.txs.filter(t => t.project === name);
      years.forEach(y => {
        const yTx = pTx.filter(t => t.year === y);
        if (yTx.length > 0) {
          const avg = Math.round(yTx.reduce((s,t) => s+t.psf, 0) / yTx.length);
          data[`${name}-${y}`] = avg;
          vol[`${name}-${y}`] = yTx.length;
        }
      });
    });
