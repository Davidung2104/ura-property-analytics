...
import PropertyAnalysisTab from './project/PropertyAnalysisTab';
import InvestmentProjectTab from './project/InvestmentProjectTab';
...
export default function Dashboard() {
  ...
  const handlePortfolioView = useCallback(async (newProj) => {
    setSec('analyze'); setATab('valuation');
    setProj(newProj);
    setProjData(null);
    setProjLoading(true);
    try {
  ...
  // ── Master project filters (shared across all project tabs) ──
  ...
  // Derive year + sale type + tenure + floor options from project transactions
  const txYears = useMemo(() => {
    const txs = projData?.txs || [];
    return [...new Set(txs.map(t => t.year))].sort();
  }, [projData]);
  const saleTypes = useMemo(() => {
    const txs = projData?.txs || [];
    return [...new Set(txs.map(t => t.saleType))].filter(Boolean).sort();
  }, [projData]);
  const tenureTypes = useMemo(() => {
    const txs = projData?.txs || [];
    return [...new Set(txs.map(t => t.tenure))].filter(Boolean).sort();
  }, [projData]);
  const floorBands = useMemo(() => {
    ...
    txs.forEach(t => {
      ...
      else if (fl >= 6 && fl <= 10) bands.add('06-10');
      else if (fl >= 11 && fl <= 15) bands.add('11-15');
      else if (fl >= 16 && fl <= 20) bands.add('16-20');
      else if (fl >= 21 && fl <= 30) bands.add('21-30');
  ...
  // Combined master filters object passed to all tabs
  ...
  // Recompute stat cards from filtered transactions
  const matchFloorDash = (fl, band) => {
  ...
  const filteredStats = useMemo(() => {
    ...
    if (floorFilter !== 'all') txs = txs.filter(t => matchFloorDash(t.floorMid || 0, floorFilter));
    ...
    return {
      ...
      medPsf: psfs.length % 2 ? psfs[m] : Math.round((psfs[m - 1] + psfs[m]) / 2),
      totalTx: txs.length,
    };
  ...
  const projProps = {
    ...
    compare: { proj, cmpPool, cmpSelected, setCmpSelected, mktData, nearbyProjects: projData?.nearbyProjects || [], selfYearPsf: projData?.yearPsf || {}, selfBedYearPsf: projData?.bedYearPsf || {}, bedOptions: rawBedOptions, masterFilters, projList, projIndex },
  ...
  return (
    <Shell>
      <div style={{ padding: '16px 28px', borderBottom: `1px solid ${T.borderLt}` }}>
        {/* ── Search-first header ── */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 12 }}>
          <h1 onClick={() => { setSec('market'); setProj(''); setProjData(null); }} style={{ fontSize: T['2xl'], fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8, letterSpacing: '-0.02em', cursor: 'pointer', whiteSpace: 'nowrap' }}>
            <span style={{ fontSize: 20 }}>🏢</span>PropIntel
          </h1>
          <ProjectSearch value={proj} projList={filteredProjList} cmpPool={cmpPool} projIndex={projIndex} onChange={(name) => { handleProjChange(name); setSec('analyze'); setATab('valuation'); }} />
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
            <button onClick={() => setSec(sec === 'portfolio' ? 'market' : 'portfolio')} style={{
              background: sec === 'portfolio' ? `${T.green}14` : 'transparent',
              border: sec === 'portfolio' ? `1px solid ${T.green}4D` : `1px solid ${T.textFaint}`,
              borderRadius: T.r, padding: '6px 14px', fontSize: T.md,
              color: sec === 'portfolio' ? T.green : T.textSub, cursor: 'pointer', fontWeight: 600,
            }}>💼 Portfolio</button>
            ...
            <button onClick={() => handleRefresh()} disabled={refreshing} style={hdrBtn(refreshing)}>{refreshing ? '⏳' : '🔄'}</button>
            <span style={{ background: mktData ? T.green : T.amber, width: 7, height: 7, borderRadius: '50%', display: 'inline-block' }} />
          </div>
        </div>

        {/* ── Project header + tabs (when project selected) ── */}
        {sec === 'analyze' && proj && <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
            <span style={{ fontSize: T.xl, fontWeight: 800, letterSpacing: '-0.02em' }}>{p.name || proj}</span>
            {p.district && <span style={{ background: `${T.purple}14`, border: `1px solid ${T.purple}30`, borderRadius: 5, padding: '2px 8px', color: T.purple, fontSize: T.sm, fontWeight: 600 }}>{p.district}</span>}
            {p.segment && <span style={{ background: `${p.segment === 'CCR' ? T.red : p.segment === 'RCR' ? T.amber : T.green}14`, border: `1px solid ${p.segment === 'CCR' ? T.red : p.segment === 'RCR' ? T.amber : T.green}30`, borderRadius: 5, padding: '2px 8px', color: p.segment === 'CCR' ? T.red : p.segment === 'RCR' ? T.amber : T.green, fontSize: T.sm, fontWeight: 600 }}>{p.segment}</span>}
            {p.tenure && <span style={{ color: T.textMute, fontSize: T.sm }}>{p.tenure}</span>}
            {p.units > 0 && <span style={{ color: T.textMute, fontSize: T.sm }}>{p.units} units</span>}
            <button onClick={() => { setProj(''); setProjData(null); setSec('market'); }} style={{ marginLeft: 'auto', background: 'none', border: `1px solid ${T.textFaint}`, borderRadius: 6, padding: '3px 10px', color: T.textMute, fontSize: T.sm, cursor: 'pointer' }}>✕ Back to Market</button>
          </div>
        ...
        {/* ── Market header + tabs (when no project) ── */}
        {sec === 'market' && <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <span style={{ fontSize: T.xl, fontWeight: 800, letterSpacing: '-0.02em' }}>Market Overview</span>
            <span style={{ color: T.textMute, fontSize: T.md }}>URA Private Residential</span>
          </div>
      ...
      {sec === 'market' && mktData && <div style={{ padding: '16px 28px' }}><div className="s">
        ...
        <StatCard label={`Rental Contracts${mktData.rentalPeriod ? ` (${mktData.rentalPeriod})` : ''}`} value={mktData.rentalTotal?.toLocaleString() || '—'} color={T.teal} icon="🏠" sub={mktData.rentalSegCounts ? `CCR ${((mktData.rentalSegCounts.CCR || 0) / 1000).toFixed(1)}k · RCR ${((mktData.rentalSegCounts.RCR || 0) / 1000).toFixed(1)}k · OCR ${((mktData.rentalSegCounts.OCR || 0) / 1000).toFixed(1)}k` : '—'} />
      ...
      {/* ══ Project Stats ══ */}
      {sec === 'analyze' && projData && <div style={{ padding: '16px 28px' }}><div className="s">
        <StatCard label="Project" value={p.name} color={T.purple} icon="🏢" sub={`${p.district} · ${p.segment}`} />
        <StatCard label={`Avg PSF${filteredStats ? ' (filtered)' : ` (${p.psfPeriod || '—'})`}`} value={`$${(filteredStats?.avgPsf || p.avgPsf).toLocaleString()}`} color={T.blue} icon="📐" sub={`Med $${(filteredStats?.medPsf || p.medPsf).toLocaleString()}`} />
        <StatCard label="Transactions" value={(filteredStats?.totalTx ?? p.totalTx).toString()} color={T.orange} icon="📋" sub={filteredStats ? `of ${p.totalTx} total` : 'All time'} />
        <StatCard label={`Avg Rent${p.rentalPeriod ? ` (${p.rentalPeriod})` : ''}`} value={p.hasRealRental ? `$${p.avgRent.toLocaleString()}/mo` : 'N/A'} color={T.teal} icon="💵" sub={p.hasRealRental ? `${p.rentalCount || ''} URA records` : 'No URA rental data'} />
        <StatCard label={`Rent PSF${p.rentalPeriod ? ` (${p.rentalPeriod})` : ''}`} value={p.hasRealRental ? `$${p.rentPsf}/sf/mo` : 'N/A'} color={T.amber} icon="📐" sub={p.hasRealRental ? 'URA rental data' : 'No URA rental data'} />
        <StatCard label="Gross Yield" value={p.hasRealRental ? `${p.yield}%` : 'N/A'} color={T.green} icon="💰" sub={p.hasRealRental ? 'Rent PSF×12 ÷ Sale PSF' : 'No URA rental data'} />
        <StatCard label="Tenure" value={p.tenure || '—'} color={T.textMute} icon="📜" sub={p.units ? `${p.units} units` : undefined} />
      </div></div>}

      {/* ══ Master Filter + Project Tabs (wrapped together for sticky to work) ══ */}
      {sec === 'analyze' && <div>
        {/* ══ Master Project Filters (sticky) ══ */}
        {projData && (
          <div style={{ position: 'sticky', top: 0, zIndex: 20, padding: '8px 28px', background: T.pageBg || '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', background: '#fff', border: `1px solid ${T.border}`, borderRadius: 8, padding: '10px 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
              {/* Bedrooms */}
              {bedOptions.length >= 2 && <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ color: T.textSub, fontSize: 12, fontWeight: 700, letterSpacing: 0.5 }}>🛏️</span>
                <div style={{ display: 'flex', gap: 3 }}>
                  {['all', ...bedOptions].map(b => (
                    <button key={b} onClick={() => setBedsFilter(b)} style={{
                      background: bedsFilter === b ? T.purple : '#f1f5f9', color: bedsFilter === b ? '#fff' : '#475569',
                      border: `1px solid ${bedsFilter === b ? T.purple : '#e2e8f0'}`, borderRadius: 5, padding: '3px 10px',
                      fontSize: 12, cursor: 'pointer', fontWeight: 600, fontFamily: b === 'all' ? T.sans : T.mono,
                    }}>{b === 'all' ? 'All' : b}</button>
                  ))}
                </div>
              </div>}
              {bedOptions.length >= 2 && <div style={{ width: 1, height: 24, background: '#e2e8f0' }} />}
              {/* Year range */}
              {txYears.length > 1 && <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ color: T.textSub, fontSize: 12, fontWeight: 700, letterSpacing: 0.5 }}>📅</span>
                <select value={effectiveYearFrom} onChange={e => setYearFrom(e.target.value)} style={{ border: '1px solid #e2e8f0', borderRadius: 5, padding: '3px 8px', fontSize: 12, color: T.text, background: yearFrom ? '#f5f3ff' : '#fff', fontFamily: T.mono }}>
                ...
                <select value={effectiveYearTo} onChange={e => setYearTo(e.target.value)} style={{ border: '1px solid #e2e8f0', borderRadius: 5, padding: '3px 8px', fontSize: 12, color: T.text, background: yearTo ? '#f5f3ff' : '#fff', fontFamily: T.mono }}>
              ...
              {saleTypes.length > 1 && <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ color: T.textSub, fontSize: 12, fontWeight: 700, letterSpacing: 0.5 }}>🏷️</span>
                  <select value={saleTypeFilter} onChange={e => setSaleTypeFilter(e.target.value)} style={{ border: '1px solid #e2e8f0', borderRadius: 5, padding: '3px 8px', fontSize: 12, color: T.text, background: saleTypeFilter !== 'all' ? '#f5f3ff' : '#fff' }}>
              ...
              {/* Tenure */}
              {tenureTypes.length > 1 && <>
                <div style={{ width: 1, height: 24, background: '#e2e8f0' }} />
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ color: T.textSub, fontSize: 12, fontWeight: 700, letterSpacing: 0.5 }}>📜</span>
                  <select value={tenureFilter} onChange={e => setTenureFilter(e.target.value)} style={{ border: '1px solid #e2e8f0', borderRadius: 5, padding: '3px 8px', fontSize: 12, color: T.text, background: tenureFilter !== 'all' ? '#f5f3ff' : '#fff' }}>
              ...
              {floorBands.length > 1 && <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ color: T.textSub, fontSize: 12, fontWeight: 700, letterSpacing: 0.5 }}>🏗️</span>
                  <select value={floorFilter} onChange={e => setFloorFilter(e.target.value)} style={{ border: '1px solid #e2e8f0', borderRadius: 5, padding: '3px 8px', fontSize: 12, color: T.text, background: floorFilter !== 'all' ? '#f5f3ff' : '#fff' }}>
                    <option value="all">All Floors</option>
                    {floorBands.map(f => <option key={f} value={f}>{f === '31+' ? 'Floor 31+' : `Floor ${f}`}</option>)}
                  </select>
                </div>
              </>}
              {/* Reset */}
              {hasMasterFilters && <>
                <div style={{ width: 1, height: 24, background: '#e2e8f0' }} />
                <button onClick={() => { setBedsFilter('all'); setYearFrom(''); setYearTo(''); setSaleTypeFilter('all'); setTenureFilter('all'); setFloorFilter('all'); }} style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 5, padding: '3px 10px', color: '#dc2626', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>✕ Reset</button>
              </>}
            </div>
          </div>
        )}
        ...
        {/* ══ Project Tab Content ══ */}
        <SectionBoundary name="Project"><div style={{ padding: '0 28px 40px', display: 'grid', gap: 16 }}>
          {projLoading && <Card><div style={{ textAlign: 'center', padding: 40 }}><div style={{ fontSize: 24, marginBottom: 8 }}>⏳</div><div style={{ color: T.textMute, fontSize: T.lg }}>Loading {proj}...</div></div></Card>}
          {!projLoading && !projData && proj && <Card><div style={{ textAlign: 'center', padding: 40 }}><div style={{ fontSize: 24, marginBottom: 8 }}>📭</div><div style={{ color: T.textMute, fontSize: T.lg }}>No URA transaction data found for {proj}.</div><div style={{ color: T.textSub, fontSize: T.md, marginTop: 8 }}>This project may be pre-launch or have insufficient records.</div></div></Card>}
