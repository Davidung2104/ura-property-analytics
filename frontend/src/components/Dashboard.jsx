...
import { fetchDashboard, fetchProject, searchSales, searchRental, fetchFilters } from "../services/api";
...
function MarketTab({ mode, data }) {
  ...
  useEffect(() => {
    fetchFilters().then(f => setTxFilters(f)).catch(() => {});
  }, []);
  ...
  // Page change handler (called by pagination buttons)
  const goPage = useCallback((pg) => { setTxPage(pg); doSearch(pg); }, [doSearch]);

  // Sort toggle helper
  const toggleSort = (col) => {
    setTxSort(prev => prev === `${col}_desc` ? `${col}_asc` : `${col}_desc`);
  };
  ...
  return (
    <div style={{ display:'grid', gap:16 }}>
      ...
      <SH icon="📋" title={s?'Sale Transaction Records':'Rental Contract Records'} sub={`${txData.total.toLocaleString()} total records. Server-side search across ALL transactions.`}/>
      <Cd>
        {/* Filters */}
        <div style={{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap',alignItems:'center'}}>
          <div style={{flex:1,minWidth:200,position:'relative'}}>
            <input value={txSearch} onChange={e=>setTxSearch(e.target.value)} placeholder='Search project name or street...' style={{background:'#fff',border:'1px solid #cbd5e1',borderRadius:8,padding:'8px 12px 8px 32px',color:'#1e293b',fontSize:12,width:'100%',outline:'none',fontFamily:fm}}/>
            <span style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',fontSize:13,opacity:0.5}}>🔍</span>
          </div>
          <select value={txDistF} onChange={e=>setTxDistF(e.target.value)} style={{background:'#fff',border:'1px solid #cbd5e1',borderRadius:8,padding:'8px 12px',color:'#1e293b',fontSize:12,cursor:'pointer',outline:'none',fontFamily:fm}}>
            <option value="">All Districts</option>
            {(txFilters.districts||[]).map(d=><option key={d} value={d}>{d}</option>)}
          </select>
          <select value={txSegF} onChange={e=>setTxSegF(e.target.value)} style={{background:'#fff',border:'1px solid #cbd5e1',borderRadius:8,padding:'8px 12px',color:'#1e293b',fontSize:12,cursor:'pointer',outline:'none',fontFamily:fm}}>
            <option value="">All Segments</option>
            {(txFilters.segments||[]).map(sg=><option key={sg} value={sg}>{sg}</option>)}
          </select>
          <select value={txTypeF} onChange={e=>setTxTypeF(e.target.value)} style={{background:'#fff',border:'1px solid #cbd5e1',borderRadius:8,padding:'8px 12px',color:'#1e293b',fontSize:12,cursor:'pointer',outline:'none',fontFamily:fm}}>
          ...
          <div style={{color:'#64748b',fontSize:11,display:'flex',alignItems:'center',gap:6}}>
            {txLoading && <span style={{display:'inline-block',width:12,height:12,border:'2px solid #e2e8f0',borderTop:'2px solid #38bdf8',borderRadius:'50%',animation:'spin 0.6s linear infinite'}}/>}
          ...
          <table>
            ...
            <tbody>
              {txData.results.map((tx,i)=><tr key={i}>
                <td style={{color:'#64748b',fontSize:11,whiteSpace:'nowrap'}}>{s?tx.date:tx.period}</td>
                <td style={{color:'#1e293b',fontWeight:600,fontSize:11,maxWidth:180,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{tx.project}</td>
                <td style={{color:'#94a3b8',fontSize:10,maxWidth:140,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{tx.street}</td>
                ...
                {s && <td style={{color:'#64748b',fontSize:10}}>{tx.type}</td>}
                <td style={{color:SC[tx.segment],fontSize:10,fontWeight:600}}>{tx.segment}</td>
              </tr>)}
              {txData.results.length===0 && !txLoading && <tr><td colSpan={s?9:9} style={{textAlign:'center',color:'#94a3b8',padding:24}}>No transactions match your filters</td></tr>}
            </tbody>
          </table>
        </div>
        {/* Pagination */}
        {txData.pages>1 && <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:12}}>
          <button onClick={()=>goPage(Math.max(1,txPage-1))} disabled={txPage<=1} style={{background:txPage<=1?'#f1f5f9':'#fff',border:'1px solid #cbd5e1',borderRadius:6,padding:'6px 14px',fontSize:11,color:txPage<=1?'#cbd5e1':'#64748b',cursor:txPage<=1?'default':'pointer'}}>← Prev</button>
          <div style={{display:'flex',gap:4,alignItems:'center'}}>
            {[...Array(Math.min(7, txData.pages))].map((_,i) => {
              ...
              return <button key={pg} onClick={()=>goPage(pg)} style={{background:pg===txPage?'#38bdf8':'#fff',color:pg===txPage?'#fff':'#64748b',border:'1px solid '+(pg===txPage?'#38bdf8':'#cbd5e1'),borderRadius:6,padding:'4px 10px',fontSize:11,cursor:'pointer',fontWeight:pg===txPage?700:400,minWidth:32}}>{pg}</button>;
            })}
          </div>
          <button onClick={()=>goPage(Math.min(txData.pages,txPage+1))} disabled={txPage>=txData.pages} style={{background:txPage>=txData.pages?'#f1f5f9':'#fff',border:'1px solid #cbd5e1',borderRadius:6,padding:'6px 14px',fontSize:11,color:txPage>=txData.pages?'#cbd5e1':'#64748b',cursor:txPage>=txData.pages?'default':'pointer'}}>Next →</button>
        </div>}
      </Cd>
    </div>
  );
...
export default function Dashboard() {
  ...
  const nearbyHm = useMemo(() => {
    ...
    cmpSelected.forEach(name => {
      ...
      if (!p) return;
      years.forEach(y => {
        // Use real per-year PSF from backend (yearPsf map)
