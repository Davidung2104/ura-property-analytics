...
export default function Dashboard() {
  ...
  const applyDashboard = useCallback((data) => {
    setMktData(data); setCmpPool(data.cmpPool || []); setProjList(data.projList || []);
  }, []);
    ...
    async function load() {
      try {
        ...
        if (data.cmpPool?.length > 0) {
          ...
          const same = data.cmpPool.filter(p => p.dist === dist && p.name !== first).slice(0, 3).map(p => p.name);
          const near = data.cmpPool.filter(p => p.dist !== dist).slice(0, 2).map(p => p.name);
          setCmpSelected([first, ...same, ...near].slice(0, 5));
        }
        setLoading(false);
  ...
  const projCoords = useMemo(() => {
    ...
    return cp ? { lat: cp.lat, lng: cp.lng } : { lat: 1.3521, lng: 103.8198 };
  }, [proj, cmpPool]);
  ...
  const projProps = {
    ...
    compare: { proj, projCoords, cmpPool, cmpSelected, setCmpSelected, mktData },
  ...
  return (
    <Shell>
      <div style={{ padding: '20px 28px', borderBottom: `1px solid ${T.borderLt}` }}>
        ...
        {sec === 'analyze' && <>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 12 }}>
            ...
            <ProjectSearch value={proj} projList={projList} cmpPool={cmpPool} onChange={handleProjChange} />
          </div>
      ...
      {sec === 'portfolio' && <SectionBoundary name="Portfolio"><div style={{ padding: '16px 28px 40px' }}>
        <Portfolio cmpPool={cmpPool} projList={projList} onViewProject={handlePortfolioView} holdings={portfolio} setHoldings={updatePortfolio} syncStatus={syncStatus} />
      </div></SectionBoundary>}
    </Shell>
  );
}
...
function ProjectSearch({ value, projList, cmpPool, onChange }) {
  ...
  const cp = (name) => cmpPool.find(p => p.name === name);
      ...
      {open && <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: '#fff', border: `1px solid ${T.purple}40`, borderRadius: T.r, boxShadow: '0 8px 24px rgba(0,0,0,0.08)', maxHeight: 340, overflowY: 'auto', zIndex: 100 }}>
        ...
        {filtered.map((name, i) => {
          ...
          return <div key={name} onClick={() => handleSelect(name)} style={{ padding: '10px 14px', cursor: 'pointer', background: isSel ? `${T.purple}10` : i % 2 === 0 ? '#fff' : T.bg, borderLeft: isSel ? `3px solid ${T.purple}` : '3px solid transparent', transition: 'background 0.1s' }} onMouseEnter={e => e.currentTarget.style.background = `${T.purple}08`} onMouseLeave={e => e.currentTarget.style.background = isSel ? `${T.purple}10` : i % 2 === 0 ? '#fff' : T.bg}>
            ...
            {!p && <div style={{ color: T.textMute, fontSize: T.xs, marginTop: 2 }}>Limited data (fewer transactions)</div>}
          </div>;
        })}
