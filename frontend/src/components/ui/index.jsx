...
export function StatCard({ label, value, color, icon, sub }) {
  return (
    <div style={{ background: T.card, borderRadius: T.rLg, padding: '16px 18px', border: `1px solid ${T.border}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <span style={{ color: T.textMute, fontSize: T.sm, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{label}</span>
        <span style={{ fontSize: 15 }}>{icon}</span>
      </div>
      <div style={{ color, fontSize: T['3xl'], fontWeight: 700, fontFamily: T.mono, letterSpacing: '-0.02em', lineHeight: 1.1 }}>{value}</div>
      {sub && <div style={{ color: T.textSub, fontSize: T.sm, marginTop: 5, fontWeight: 500, letterSpacing: '0.01em' }}>{sub}</div>}
    </div>
  );
}
...
export function SectionHeader({ icon, title, sub }) {
  return (
    <div style={{ marginBottom: 14 }}>
      <h3 style={{ color: T.text, fontSize: T['2xl'], fontWeight: 700, margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 8, letterSpacing: '-0.02em' }}>
