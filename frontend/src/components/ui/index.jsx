  ...
  return (
    <div style={{ background: T.card, borderRadius: T.rLg, padding: '14px 16px', border: `1px solid ${T.border}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
        <span style={{ color: T.textSub, fontSize: T.sm, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{label}</span>
        <span style={{ fontSize: 14 }}>{icon}</span>
      </div>
      <div style={{ color, fontSize: T['2xl'], fontWeight: 700, fontFamily: T.mono, letterSpacing: '-0.01em' }}>{value}</div>
      {sub && <div style={{ color: T.textSub, fontSize: T.sm, marginTop: 3, fontWeight: 500, letterSpacing: '0.01em' }}>{sub}</div>}
    </div>
  );
}
...
export function SectionHeader({ icon, title, sub }) {
  return (
    <div style={{ marginBottom: 12 }}>
      <h3 style={{ color: T.text, fontSize: T.xl, fontWeight: 700, margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 8, letterSpacing: '-0.01em' }}>
        <span style={{ fontSize: 17 }}>{icon}</span>{title}
      </h3>
      {sub && <p style={{ color: T.textSub, fontSize: T.md, margin: 0, lineHeight: 1.5, fontWeight: 500 }}>{sub}</p>}
    </div>
  );
}
