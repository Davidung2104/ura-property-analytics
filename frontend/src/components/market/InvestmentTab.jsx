...
export default function InvestmentTab({ data }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      {/* Investment Quadrant */}
      <SectionHeader title="District Investment Quadrant" sub="X = Price CAGR (capital growth), Y = Gross Yield (income). Top-right = best total return." />
      <Card>
        <div style={{ height: 340 }}><ResponsiveContainer width="100%" height="100%"><ScatterChart margin={{ top: 20, right: 30, bottom: 20, left: 10 }}><CartesianGrid strokeDasharray="3 3" stroke={T.border} /><XAxis type="number" dataKey="cagr" name="Price CAGR %" tick={{ fill: T.textSub, fontSize: T.sm }} axisLine={false} tickFormatter={v => `${v}%`} domain={['auto', 'auto']} /><YAxis type="number" dataKey="y" name="Gross Yield %" tick={{ fill: T.textSub, fontSize: T.sm }} axisLine={false} tickFormatter={v => `${v}%`} domain={['auto', 'auto']} /><Tooltip content={({ active, payload }) => {
          ...
          return <div style={{ background: '#1b2d4bf0', padding: '10px 14px', borderRadius: T.r, border: '1px solid #c4cdd8' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
          {[
            { c: T.green, label: 'Top Right: BEST', desc: 'High growth + high yield' },
            { c: T.amber, label: 'Top Left: INCOME', desc: 'High yield, slower growth' },
            { c: T.blue, label: 'Bottom Right: GROWTH', desc: 'High CAGR, lower yield' },
            { c: T.red, label: 'Bottom Left: AVOID', desc: 'Low on both axes' },
          ].map(q => <div key={q.label} style={{ background: `${q.c}10`, borderRadius: T.r, padding: '8px 12px', border: `1px solid ${q.c}30` }}><span style={{ color: q.c, fontSize: T.md, fontWeight: 600 }}>{q.label}</span><span style={{ color: T.textSub, fontSize: T.md }}> — {q.desc}</span></div>)}
        </div>
      </Card>

      <SectionHeader title="Gross Rental Yield by District" sub="Yield = (Monthly Rent PSF × 12) ÷ Sale PSF. Green ≥ 3%, yellow ≥ 2.5%, red < 2.5%." />
      ...
      <SectionHeader title="Buy Price vs Rent Collected" sub="The wider the gap between blue (cost) and green (income), the longer your payback period." />
