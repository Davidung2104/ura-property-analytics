      ...
      <div className="g2">
        <Card>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <SnapBox label="AVG PSF" value={`$${data.avgPsf?.toLocaleString() || '—'}`} color={T.blue} sub={data.yoyPct != null ? `${data.yoyPct > 0 ? '+' : ''}${data.yoyPct}% YoY` : '—'} subColor={T.lime} />
            ...
            <SnapBox label="MEDIAN PSF" value={`$${data.medPsf?.toLocaleString() || '—'}`} color={T.indigo} sub="Right-skewed" />
...
function SnapBox({ label, value, color, sub, subColor }) {
  return (
    <div style={{ background: T.borderLt, borderRadius: 10, padding: 12, textAlign: 'center' }}>
