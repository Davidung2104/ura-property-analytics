...
export default function PerformanceTab({ data }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      {/* Bar chart */}
      <SectionHeader icon="📈" title={`District ${cagrYrs}-Year CAGR`} sub={`Annualised price growth from ${sYear} to ${eYear}. Sorted by ${sortBy === 'cagr' ? 'CAGR' : sortBy}.`} />
      <Card>
      ...
      {/* Performance table */}
      <SectionHeader icon="🏆" title="District Performance Table" sub={`All districts ranked by ${sortBy === 'cagr' ? 'CAGR' : sortBy}. Click column headers to sort. ${sYear} → ${eYear} (${cagrYrs}yr).`} />
      ...
      {/* Absolute PSF change chart */}
      <SectionHeader icon="💵" title="Absolute PSF Change" sub={`Dollar gain/loss per square foot from ${sYear} to ${eYear}.`} />
      <Card>
