...
export default function PerformanceTab({ data }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <SectionHeader title={`District ${cagrYrs}-Year Growth`} sub={`${sYear} → ${eYear}. Left = annualised CAGR (%). Right = absolute PSF gain/loss ($).`} />
      <div className="g2">
        <Card>
      ...
      <SectionHeader title="District Performance Table" sub={`All districts ranked by ${sortBy === 'cagr' ? 'CAGR' : sortBy}. Click column headers to sort. Click a row to filter projects below. ${sYear} → ${eYear} (${cagrYrs}yr).`} />
      <Card style={{ padding: 0, overflow: 'hidden' }}>
                  ...
                  {COL_DEFS.map(col => {
                    ...
                    return (
                      <td key={col.key} style={{
                        ...
                        {col.key === 'd' && row.lowConf && <span title="Low confidence" style={{ marginLeft: 4, fontSize: T.xs, color: T.amber, fontWeight: 700 }}>!</span>}
                      </td>
                    );
                  })}
                </tr>
        ...
        <div style={{ padding: '10px 16px', borderTop: `1px solid ${T.border}`, color: T.textMute, fontSize: T.sm }}>
          ...
          {rows.filter(r => r.lowConf).length > 0 && <span style={{ color: T.amber }}> · Faded = {'<'}3 tx in start/end year</span>}
        </div>
      </Card>

      {/* ── Project Performance Table ── */}
      {(data?.projPerf?.length > 0) && <>
        <SectionHeader title={projFilter !== 'all' ? `District ${projFilter} — Project Performance` : 'Project Performance Table'} sub={`${projFilter !== 'all' ? projRows.length : data?.projPerf?.length || 0} projects with transactions in both ${sYear} and ${eYear}. Min 5 total tx. Click headers to sort.`} />
        <Card style={{ padding: 0, overflow: 'hidden' }}>
          {/* District filter */}
                    ...
                    {PROJ_COLS.map(col => {
                      ...
                      return (
                        <td key={col.key} style={{
                          ...
                          {col.fmt ? col.fmt(val) : val}
                          {col.key === 'name' && row.lowConf && <span title="Low confidence" style={{ marginLeft: 4, fontSize: T.xs, color: T.amber, fontWeight: 700 }}>!</span>}
                        </td>
                      );
                    })}
                  </tr>
          ...
          <div style={{ padding: '10px 16px', borderTop: `1px solid ${T.border}`, color: T.textMute, fontSize: T.sm }}>
            ...
            {projRows.filter(r => r.lowConf).length > 0 && <span style={{ color: T.amber }}> · Faded = {'<'}2 tx in start/end year</span>}
            {' · '}Total = CAGR + Yield
          </div>
        </Card>
      </>}
