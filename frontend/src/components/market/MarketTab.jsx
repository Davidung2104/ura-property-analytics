  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <InsightBar items={s ? [
        ...
        <span key="a">Avg rent <span style={{ color: T.lime, fontWeight: 700, fontFamily: T.mono }}>{data?.hasRealRental ? `$${data?.avgRent?.toLocaleString() || '—'}/mo` : 'N/A'}</span> {data?.hasRealRental ? '(URA data)' : '(no URA rental data)'}</span>,
      ...
      <SectionHeader title={s ? 'Sale Price Trend' : 'Rental Trend'} sub={s ? 'Average & median PSF with YoY growth rate.' : 'Average & median monthly rent with QoQ growth rate.'} />
      ...
      <div className="g2">
        <Card><SectionHeader title={s ? 'Sales by Segment' : 'Rentals by Segment'} sub={s ? 'Transaction volume and average PSF by market segment.' : 'Rental volume and average rent by market segment.'} />
        ...
        <Card><SectionHeader title={s ? 'Most Traded Projects' : 'Most Rented Projects'} sub={s ? 'Highest transaction volume projects.' : 'Highest rental contract volume.'} />
      ...
      <SectionHeader title={s ? 'District PSF Trends' : 'District Rent PSF Trends'} sub={s ? 'Quarterly average PSF for top 5 districts.' : 'Quarterly average rent PSF for top 5 districts.'} />
      ...
      <SectionHeader title={s ? 'Current PSF by District' : 'Current Rent PSF by District'} sub={s ? 'Average PSF across all districts.' : 'Average rent PSF by district.'} />
      ...
      {s && data?.distTopPsf?.length > 0 && <>
        <SectionHeader title="Highest PSF by District" sub="Click any district to see top projects ranked by PSF. Uses latest year average." />
        <Card>
      ...
      <SectionHeader title={s ? 'Price vs Unit Size' : 'Rent PSF vs Unit Size'} sub={s ? 'Inverse relationship: larger units tend to have lower PSF.' : 'Same inverse pattern in rentals.'} />
      ...
      <SectionHeader title={s ? 'Quarterly Sales Volume' : 'Quarterly Rental Contracts'} sub={s ? 'Per-quarter dollar volume (last 12 quarters).' : 'Per-quarter contract count (last 12 quarters).'} />
      ...
      <SectionHeader title={s ? 'Sale Transaction Records' : 'Rental Contract Records'} sub={`${txData.total.toLocaleString()} total records. Server-side search across ALL transactions.`} />
      <Card>
