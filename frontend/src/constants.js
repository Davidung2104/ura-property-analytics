...
export const S = {
  // Axis tick
  tick:     { fill: T.textSub, fontSize: T.sm },
  tickSm:   { fill: T.textSub, fontSize: T.xs },
  ...
  togBtn: (active, color = T.purple) => ({
    ...
    color: active ? color : T.textSub, cursor: 'pointer', fontWeight: 600,
  }),
  // Section toggle
  secBtn: (active, color) => ({
    ...
    color: active ? color : T.textSub, fontSize: T.lg, fontWeight: 600, cursor: 'pointer',
  }),
  // Filter select
  filterSel: (active) => ({
...
// ── Bed-size mapping (must match backend BED_RANGES) ──
