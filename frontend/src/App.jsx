...
export default function App() {
  ...
  const metrics = useMemo(() => {
    ...
    const avgPsf = total > 0 ? filteredData.reduce((sum, t) => sum + (t.psf || 0), 0) / total : 0;
  ...
  const timeSeriesData = useMemo(() => {
    ...
    filteredData.forEach(t => {
      ...
      grouped[month].psfValues.push(t.psf);
      grouped[month].count++;
    });
  ...
  const floorAnalysis = useMemo(() => {
    ...
    filteredData.forEach(t => {
      ...
      grouped[t.floorRange].push(t.psf);
    });
  ...
  const comparisonData = useMemo(() => {
    ...
    filteredData.filter(t => comparisonProjects.includes(t.project)).forEach(t => {
      ...
      grouped[month][t.project].sum += t.psf;
  ...
  const clearFilters = () => { setYearFilter(''); setProjectFilter(''); setDistrictFilter(''); setFloorFilter(''); setCurrentPage(1); };
  ...
  const exportToCSV = () => {
    const csv = [['Project','Street','District','Floor','Area','Price','PSF','Date','Segment'], ...sortedData.map(t => [t.project,t.street,t.district,t.floorRange,t.areaNum,t.priceNum,t.psf,t.contractDate,t.marketSegment])].map(r => r.join(',')).join('\n');
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="bg-white border-b sticky top-0 z-50 shadow-sm">
        ...
        <div className="bg-slate-50 border-t px-4 py-3">
          <div className="max-w-7xl mx-auto flex items-center gap-3 flex-wrap">
            ...
            <select value={projectFilter} onChange={e => { setProjectFilter(e.target.value); setCurrentPage(1); }} className="px-3 py-1.5 text-sm border rounded-lg bg-white max-w-[200px]"><option value="">All Projects</option>{projectNames.slice(0, 100).map(p => <option key={p} value={p}>{p}</option>)}</select>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-5 rounded-xl shadow-sm border"><div className="text-slate-500 text-sm mb-2">Average PSF</div><div className="text-2xl font-bold">${Math.round(metrics.avgPsf).toLocaleString()}</div><div className="text-xs text-slate-400">Per Square Foot</div></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
            <div className="p-6 border-b"><h3 className="text-lg font-semibold">Floor Premium</h3></div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border">
          <div className="flex justify-between mb-4"><div><h3 className="text-lg font-semibold">Project Comparison</h3><p className="text-sm text-slate-500">Select up to 5 projects</p></div><select onChange={e => e.target.value && toggleComparison(e.target.value)} value="" className="px-3 py-1.5 text-sm border rounded-lg"><option value="">+ Add project...</option>{projectNames.filter(p => !comparisonProjects.includes(p)).slice(0, 100).map(p => <option key={p} value={p}>{p}</option>)}</select></div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50"><tr>
                ...
                <th className="px-4 py-3 text-right cursor-pointer hover:bg-slate-100" onClick={() => handleSort('areaNum')}>Area <SortIcon field="areaNum" /></th>
              ...
              <tbody className="divide-y">{paginatedData.map((t, i) => <tr key={i} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3 text-right font-mono text-slate-600">{t.areaNum} sqm</td>
                <td className="px-4 py-3 text-right font-mono font-medium">{formatCompact(t.priceNum)}</td>
                <td className="px-4 py-3 text-right font-mono text-sky-600 font-medium">${t.psf?.toLocaleString()}</td>
