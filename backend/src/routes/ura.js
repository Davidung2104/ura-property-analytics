...
router.get('/rental/search', async (req, res, next) => {
  try {
    const { q, district, segment, bedrooms, areaSqft, page, limit, sort } = req.query;
    const data = await searchRental({
      q, district, segment, bedrooms, areaSqft,
      page: parseInt(page) || 1,
      limit: Math.min(parseInt(limit) || 50, 200),
      sort: sort || 'date_desc',
    });
...
// Write rate limit: 30 PUT/min per IP (prevents disk-fill attacks)
 ...
 * Rate limited: 30 writes/min per IP
 */
