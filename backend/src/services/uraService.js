...
class Agg {
  ...
  build(rentalData) {
    ...
    // 12M rolling: sum last 4 quarters for avg/med PSF (matches project-level logic)
    const last4Q = qtrs.slice(-4);
    const rolling12M = { s: 0, n: 0, p: [] };
    for (const q of last4Q) {
      const qb = this.byQtr[q];
      if (qb) { rolling12M.s += qb.s; rolling12M.n += qb.n; }
    }
    // Collect price samples for median: from byYear for years covered by last 4 quarters
    const rollingYears = [...new Set(last4Q.map(q => '20' + q.slice(0, 2)))];
    for (const ry of rollingYears) {
      if (this.byYear[ry]?.p) rolling12M.p.push(...this.byYear[ry].p);
    }

    const use12M = rolling12M.n >= 50; // enough data for 12M rolling
    const avgPsf = use12M ? avg(rolling12M.s, rolling12M.n) : (this.byYear[latY] ? avg(this.byYear[latY].s, this.byYear[latY].n) : avg(Object.values(this.byYear).reduce((s,y) => s + y.s, 0), this.total));
    const psfPeriod = use12M ? '12M' : latY;

    // Latest-year samples for area/PSF estimates (fallback to all if < 50 samples)
    ...
    // Median from 12M rolling price samples, fallback to latest year
    const medPsf = use12M && rolling12M.p.length > 0 ? med(rolling12M.p) : (this.byYear[latY]?.p?.length > 0 ? med(this.byYear[latY].p) : med(this.samples.map(s => s.psf)));
    ...
    // Use last 4 quarters for BOTH sale PSF and rental PSF (consistent 12M window)
    if (hasRental) {
      computedYield = {};

      // Last 4Q sale PSF by segment
      const salePsf4Q = {};
      for (const q of last4Q) {
        const qb = this.byQtr[q];
        if (!qb?.bySeg) continue;
        for (const [seg, data] of Object.entries(qb.bySeg)) {
          if (!salePsf4Q[seg]) salePsf4Q[seg] = { s: 0, n: 0 };
          salePsf4Q[seg].s += data.s;
          salePsf4Q[seg].n += data.n;
        }
      }

      // Last 4Q rental PSF by segment (from rentalStore)
      // NOTE: Sale quarters use uppercase "24Q1", rental uses lowercase "24q1" — must convert
      const last4QSetLower = new Set(last4Q.map(q => q.replace('Q', 'q')));
      const rentPsf4Q = {};
      for (const r of rentalStore) {
        if (!last4QSetLower.has(r.d)) continue;
        if (!rentPsf4Q[r.sg]) rentPsf4Q[r.sg] = { s: 0, n: 0 };
        rentPsf4Q[r.sg].s += r.rp;
        rentPsf4Q[r.sg].n++;
      }
      ...
      for (const seg of ['CCR', 'RCR', 'OCR']) {
        const sp = salePsf4Q[seg]?.n > 0 ? avg(salePsf4Q[seg].s, salePsf4Q[seg].n) : 0;
        // Use precise average for rental PSF (not Math.round — values are ~$2-5/sf/mo)
        const rp = rentPsf4Q[seg]?.n > 0 ? +(rentPsf4Q[seg].s / rentPsf4Q[seg].n).toFixed(2) : 0;
        if (sp > 0 && rp > 0) {
          computedYield[seg] = (rp * 12) / sp;
          fallbackYield += computedYield[seg] * (salePsf4Q[seg]?.n || 1);
          fallbackCount += salePsf4Q[seg]?.n || 1;
        }
      }
      // For segments with no rental data, use weighted average of computed segments (not hardcoded)
      ...
      console.log('📊 Real yields from URA (4Q):', Object.entries(computedYield).map(([k,v])=>`${k}: ${(v*100).toFixed(2)}%`).join(', '));
    } else {
    ...
    if (rentalStore.length > 0) {
      ...
      // Fallback: estimate from recent sales samples
      const rEsts = recentSamples.map(s => Math.round(estRent(s.psf, s.seg) * s.area));
    ...
    const yd = [...ydAll].sort((a,b) => b.y - a.y).slice(0, 8);
    ...
    const projPerf = Object.values(this.byProj).map(p => {
      ...
      const yld = rp && eA > 0 ? +((rp.avgRentPsf * 12 / eA) * 100).toFixed(2) : +(getYield(p.seg) * 100).toFixed(2);
      const totalReturn = +(cagr + yld).toFixed(2);
      return {
    ...
    const mktRentTx = rentalStore.length > 0
      ...
      : mktSaleTx.slice(0, 100).map(tx => {
          ...
          return { date: tx.date, project: tx.project, district: tx.district, segment: tx.segment, unit: tx.unit,
            area: tx.area.toLocaleString(), bedrooms: '', floor: tx.floor, rent, rentPsf: +(rent / tx.area).toFixed(2) };
        });
    ...
    for (const p of Object.values(this.byProj)) {
      ...
      const rp = hasRental ? rentalData?.byProject?.[p.name] : null;
      const yld = rp && psfVal > 0 ? +((rp.avgRentPsf * 12 / psfVal) * 100).toFixed(2) : +(getYield(p.seg) * 100).toFixed(2);
    ...
    return {
      ...
      rentalPeriod: (() => { const rqs = [...new Set(rentalStore.map(r => r.d))].sort(); return rqs.length > 1 ? `${rqs[0]}–${rqs[rqs.length-1]}` : rqs[0] || ''; })(),
      ...
      avgYield: ydAll.length > 0 ? +(ydAll.reduce((s,d) => s + d.y, 0) / ydAll.length).toFixed(2) : 0,
...
function _rebuildProjYearData() {
  ...
  for (const [name, meta] of Object.entries(idx)) {
    ...
    projYearData[name] = {
      ...
      n: meta.n, type: meta.type, psf: meta.psf, yield: meta.yield, yearPsf,
    };
  }
  console.log(`🔗 Rebuilt projYearData: ${Object.keys(projYearData).length} projects`);
}
...
export async function getProjectData(projectName) {
  ...
  // Fallback to projIndex for projects not in top-30 cmpPool
...
function buildProjectResult(p, pool) {
  ...
  // Last 12 months window — shared by PSF calc, floor premium, and sale tx filter
  const now = new Date();
  const m12 = `${now.getFullYear() - 1}-${String(now.getMonth() + 1).padStart(2, '0')}`;
  const recentTx = txs.filter(t => t.date >= m12);

  // Current PSF: prefer last 12 months, fallback to latest year, then all-time
  // Use any available transactions in window (no minimum threshold — label should reflect actual period)
  const latestYear = years[years.length - 1];
  const latYearTx = txs.filter(t => t.year === latestYear);
  const psfSource = recentTx.length > 0 ? recentTx : latYearTx.length > 0 ? latYearTx : txs;
  const psfPeriod = psfSource === recentTx ? '12M' : psfSource === latYearTx ? latestYear : 'all';
  ...
  const projRentTx = realProjRentals.length > 0
    ...
    : txs.slice(0, 10).map(t => {
        ...
        return {
          date: `${t.year}-${String(t.month).padStart(2,'0')}`,
          address: t.floorRange || '-',
          area: t.area.toLocaleString(), areaSqf: t.area, bedrooms: '', rent, psf: +(rent / t.area).toFixed(2),
          leaseDate: '', contracts: 0,
        };
      });
  ...
  const nearbyProjects = (() => {
    ...
    for (const [name, pd] of Object.entries(projYearData)) {
      ...
      // Estimate rent from pre-built maps
      let rent = 0;
      const rp = rentalByProj[name];
      if (rp) {
        rent = Math.round(rp.total / rp.n / 100) * 100;
      } else if (pd.psf > 0) {
        const ap = areaByProj[name];
        const nearAvgA = ap ? Math.round(ap.total / ap.n) : avgA;
        rent = Math.round(pd.psf * getYield(pd.seg) / 12 * nearAvgA / 100) * 100;
      }
      const entry = { name, ...pd, rent, rel: projStreet && pd.street === projStreet ? 'street' : 'district' };
  ...
  return {
    ...
    rentalBedrooms: [...new Set(realProjRentals.map(r => r.br).filter(b => b && b !== ''))].sort((a, b) => parseInt(a) - parseInt(b)),
  };
}
...
export function buildFilteredDashboard(filters = {}) {
  ...
  // 12M rolling: sum last 4 quarters (matches main dashboard)
  const last4Q = qtrs.slice(-4);
  const rolling12M = { s: 0, n: 0, p: [] };
  for (const q of last4Q) {
    const qb = byQtr[q];
  ...
  const rollingYears = [...new Set(last4Q.map(q => '20' + q.slice(0, 2)))];
  for (const ry of rollingYears) {
    if (byYear[ry]?.p) rolling12M.p.push(...byYear[ry].p);
  }

  const use12M = rolling12M.n >= 50;
  const avgPsf = use12M ? avg(rolling12M.s, rolling12M.n) : (latY && byYear[latY] ? avg(byYear[latY].s, byYear[latY].n) : avg(allPsf.reduce((s, v) => s + v, 0), totalTx));
  const medPsf = use12M && rolling12M.p.length > 0 ? med(rolling12M.p) : (latY && byYear[latY]?.p?.length > 0 ? med(byYear[latY].p) : med(allPsf));
  const psfPeriod = use12M ? '12M' : latY;
  ...
  const projPerf = Object.values(byProj).map(p => {
    ...
    const yld = rp && eA > 0 ? +((rp.totalPsf / rp.n * 12 / eA) * 100).toFixed(2) : +(getYield(p.seg) * 100).toFixed(2);
    const totalReturn = +(cagr + yld).toFixed(2);
    return {
  ...
  return {
    ...
    rentalTotal: rCount,
    rentalPeriod: (() => { const rqs = [...new Set(rentals.map(r => r.d))].sort(); return rqs.length > 1 ? `${rqs[0]}–${rqs[rqs.length-1]}` : rqs[0] || ''; })(),
    rentalSegCounts: { CCR: rSeg.find(s => s.name === 'CCR')?.count || 0, RCR: rSeg.find(s => s.name === 'RCR')?.count || 0, OCR: rSeg.find(s => s.name === 'OCR')?.count || 0 },
