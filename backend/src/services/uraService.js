 ...
 * Creates per-project and market-wide area→bedroom breakpoints.
...
function buildBedroomModel() {
  ...
  // Build breakpoints from sorted bedroom medians
  const buildBreakpoints = (areasByBed) => {
    const entries = Object.entries(areasByBed)
      .filter(([, areas]) => areas.length >= 3) // need ≥3 records for statistical validity
      .map(([br, areas]) => {
        const sorted = [...areas].sort((a, b) => a - b);
        const median = sorted[Math.floor(sorted.length / 2)];
        return { br, median, count: areas.length };
      })
      .sort((a, b) => a.median - b.median);

    if (entries.length === 0) return null;

    // Create midpoint breakpoints between consecutive bedroom medians
    // e.g. 1BR median=550, 2BR median=850 → breakpoint at 700
    const breakpoints = [];
    for (let i = 0; i < entries.length; i++) {
      const upper = i < entries.length - 1
        ? Math.round((entries[i].median + entries[i + 1].median) / 2)
        : Infinity;
      breakpoints.push({ br: entries[i].br, maxArea: upper, median: entries[i].median });
    }
    return breakpoints;
  };

  const projBreakpoints = {};
  for (const [proj, areasByBed] of Object.entries(projAreas)) {
    const bp = buildBreakpoints(areasByBed);
    if (bp && bp.length >= 2) projBreakpoints[proj] = bp; // need ≥2 bedroom types to be useful
  }

  const mktBreakpoints = buildBreakpoints(mktAreas);

  bedroomModel = { projBreakpoints, mktBreakpoints };
  const projCount = Object.keys(projBreakpoints).length;
  const mktBeds = mktBreakpoints ? mktBreakpoints.map(b => `${b.br}BR≤${b.maxArea === Infinity ? '∞' : b.maxArea}`).join(', ') : 'none';
  console.log(`  🛏️  Bedroom model: ${projCount} projects with project-level, market=[${mktBeds}]`);
}

/**
 * Infer bedroom count from area, using project-specific model first, then market-wide.
 * @param {string} project - Project name
 * @param {number} area - Area in sqft
 * @returns {string} Bedroom count (e.g. '1', '2', '3') or '' if unknown
 */
function inferBedrooms(project, area) {
  if (!bedroomModel || area <= 0) return '';

  // Try project-specific breakpoints first
  const bp = bedroomModel.projBreakpoints[project] || bedroomModel.mktBreakpoints;
  if (!bp) return '';

  for (const b of bp) {
    if (area <= b.maxArea) return b.br;
  }
  // Shouldn't reach here (last breakpoint has maxArea=Infinity), but fallback
  return bp[bp.length - 1]?.br || '';
}
...
function buildProjectResult(p, pool) {
  ...
  const txs = (p.transaction || []).map(tx => {
    ...
    return { year: String(d.year), quarter: d.quarter, month: d.month, date: `${d.year}-${String(d.month).padStart(2,'0')}`, area, price, psf, floorRange: fl.band, floorMid: fl.mid, saleType: tx.typeOfSale === '1' ? 'New Sale' : tx.typeOfSale === '2' ? 'Sub Sale' : 'Resale', size: area, floor: fl.band, beds };
  }).filter(Boolean);
  ...
  const projTx = txs.map(t => ({
    ...
    area: t.area, price: t.price, psf: t.psf, type: t.saleType, beds: t.beds,
  }));
