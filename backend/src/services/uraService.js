...
class Agg {
  ...
  build(rentalData) {
    ...
    // Median from latest year price samples (not all-time reservoir)
    const medPsf = this.byYear[latY]?.p?.length > 0 ? med(this.byYear[latY].p) : med(this.samples.map(s => s.psf));
    ...
    // Use LATEST YEAR segment PSF for accurate yield calculation
    if (hasRental) {
      computedYield = {};
      for (const seg of ['CCR', 'RCR', 'OCR']) {
        const segData = this.bySeg[seg];
        const salePsf = segData ? latOr(segData) : 0;
...
function buildProjectResult(p, pool) {
  ...
  const psfPeriod = psfSource === recentTx ? '12mo' : psfSource === latYearTx ? latestYear : 'all';
  ...
  return {
    projInfo: {
      ...
      units: txs.length, avgPsf: avgP, psfPeriod, medPsf: med(txs.map(t => t.psf)),
...
export function buildFilteredDashboard(filters = {}) {
  ...
  const medPsf = latY && byYear[latY]?.p?.length > 0 ? med(byYear[latY].p) : med(allPsf);
  const avgArea = recentSamples.length > 0 ? Math.round(recentSamples.reduce((s, p) => s + p.area, 0) / recentSamples.length) : 0;
