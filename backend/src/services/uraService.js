 ...
 * Creates per-project and market-wide median area per bedroom count.
...
function buildBedroomModel() {
  ...
  // Build median lookup per bedroom type
  const buildMedians = (areasByBed) => {
    const entries = Object.entries(areasByBed)
      .filter(([, areas]) => areas.length >= 3) // need ≥3 records for validity
      .map(([br, areas]) => {
  ...
  for (const [proj, areasByBed] of Object.entries(projAreas)) {
    const m = buildMedians(areasByBed);
    if (m) projMedians[proj] = m;
  }

  const mktMedians = buildMedians(mktAreas);

  bedroomModel = { projMedians, mktMedians };
  const projCount = Object.keys(projMedians).length;
  const mktBeds = mktMedians ? mktMedians.map(b => `${b.br}BR@${b.median}sf`).join(', ') : 'none';
  console.log(`  🛏️  Bedroom model: ${projCount} projects with project-level, market=[${mktBeds}]`);
}

/**
 * Infer bedroom count from area using closest-median matching.
 * Finds which bedroom type's median area is closest to the given area.
...
function inferBedrooms(project, area) {
  ...
  // Try project-specific medians first, fall back to market-wide
  const medians = bedroomModel.projMedians[project] || bedroomModel.mktMedians;
  if (!medians) return '';

  // Find bedroom type with closest median area
  let best = medians[0];
  let bestDist = Math.abs(area - best.median);
  for (let i = 1; i < medians.length; i++) {
    const dist = Math.abs(area - medians[i].median);
    if (dist < bestDist) {
      bestDist = dist;
      best = medians[i];
    }
...
function buildProjectResult(p, pool) {
  ...
  const nearbyProjects = (() => {
    ...
    for (const r of salesStore) {
      ...
      if (!projBedYearPsf[r.p][beds]) projBedYearPsf[r.p][beds] = {};
      if (!projBedYearPsf[r.p][beds][year]) projBedYearPsf[r.p][beds][year] = { s: 0, n: 0 };
      projBedYearPsf[r.p][beds][year].s += r.ps;
      projBedYearPsf[r.p][beds][year].n++;
    }
  ...
  return {
    ...
    bedYearPsf: (() => {
      ...
      for (const t of txs) {
        if (!t.beds) continue;
        if (!byp[t.beds]) byp[t.beds] = {};
        if (!byp[t.beds][t.year]) byp[t.beds][t.year] = { s: 0, n: 0 };
        byp[t.beds][t.year].s += t.psf;
        byp[t.beds][t.year].n++;
      }
      const result = {};
