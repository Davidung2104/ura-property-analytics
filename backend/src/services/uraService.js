...
let cacheTime = null;
const CACHE_TTL = Infinity; // Only refresh via POST /api/refresh (set CACHE_TTL_HOURS env var to enable auto-expiry)
// FIX #4: Optional auto-refresh interval via env var
const CACHE_TTL_MS = process.env.CACHE_TTL_HOURS
...
let salesStore = [];   // All sales tx, compact format
let rentalStore = [];  // All rental tx from PMI_Resi_Rental
// FIX #6: Memory bounds — cap transaction stores to prevent unbounded growth
const MAX_SALES_RECORDS = 250_000;   // ~25MB at ~100 bytes each
const MAX_RENTAL_RECORDS = 100_000;  // ~10MB
...
async function fetchRentalData() {
  ...
  // FIX #1: Safe division helper — returns 0 instead of NaN when divisor is 0
  const safeDiv = (num, den, decimals = 2) => den > 0 ? +(num / den).toFixed(decimals) : 0;
...
// FIX #3: Mutex to prevent concurrent refresh race conditions
...
export async function buildDashboardData(force = false) {
  ...
  // FIX #3: If a refresh is already in progress, wait for it instead of starting a second one
  if (_refreshLock) {
...
async function _doBuildDashboardData(force) {
  ...
  // FIX #6: Enforce memory bounds — keep most recent records if stores exceed caps
  if (salesStore.length > MAX_SALES_RECORDS) {
    console.warn(`⚠️ salesStore exceeds cap (${salesStore.length} > ${MAX_SALES_RECORDS}), trimming oldest records`);
    salesStore = salesStore.slice(0, MAX_SALES_RECORDS);
  }
  if (rentalStore.length > MAX_RENTAL_RECORDS) {
    console.warn(`⚠️ rentalStore exceeds cap (${rentalStore.length} > ${MAX_RENTAL_RECORDS}), trimming oldest records`);
    rentalStore = rentalStore.slice(0, MAX_RENTAL_RECORDS);
  }
...
function _rebuildProjYearData() {
  ...
  for (const [name, meta] of Object.entries(idx)) {
    ...
    const rent = rp && rp.n > 0 ? Math.round(rp.totalPsf / rp.n) : 0; // avg rent PSF for nearby comparisons
    projYearData[name] = {
...
export function buildFilteredDashboard(filters = {}) {
  ...
  const rDistLine = sDistLine.map(row => {
    ...
    const rqKey = row.q.replace('Q', 'q');
    topDist.forEach(d => {
      if (hasRental && rByDist[d]?.byQ?.[rqKey]?.n > 0) {
        r[d] = +(rByDist[d].byQ[rqKey].totalPsf / rByDist[d].byQ[rqKey].n).toFixed(2);
      } else if (hasRental && rByDist[d]?.n > 0) {
