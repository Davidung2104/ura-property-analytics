...
class Agg {
  ...
  build(rentalData) {
    ...
    if (hasRental) {
      ...
      const last4QSet = new Set(last4Q);
      const rentPsf4Q = {};
      for (const r of rentalStore) {
        if (!last4QSet.has(r.d)) continue;
      ...
      for (const seg of ['CCR', 'RCR', 'OCR']) {
        ...
        const rp = rentPsf4Q[seg]?.n > 0 ? avg(rentPsf4Q[seg].s, rentPsf4Q[seg].n) : 0;
        if (sp > 0 && rp > 0) {
    ...
    const mktRentTx = rentalStore.length > 0
      ? rentalStore.slice(0, 500).map(r => ({
          ...
          area: r.a, floor: 0, rent: r.rn, rentPsf: r.rp,
        }))
      ...
      for (const p of projects) {
        ...
        for (const r of rentals) {
          ...
          rentalStore.push({
            ...
            a: areaSqf, rn: monthlyRent, rp: rentPsf,
...
function buildProjectResult(p, pool) {
  ...
  // Current PSF: prefer last 12 months ≥3 tx, fallback to latest year ≥3, then all-time
  const latestYear = years[years.length - 1];
  const latYearTx = txs.filter(t => t.year === latestYear);
  const psfSource = recentTx.length >= 3 ? recentTx : latYearTx.length >= 3 ? latYearTx : txs;
...
export async function searchRental(opts = {}) {
  ...
  return {
    ...
    results: slice.map(r => ({
      ...
      area: r.a, rent: r.rn, rentPsf: r.rp,
...
export function getFilterOptions() {
  ...
  return { districts, segments, types, tenures, propertyTypes, years };
}
