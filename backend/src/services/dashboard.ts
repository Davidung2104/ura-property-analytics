 ...
 */
import { saveToDisk, loadFromDisk, getCacheStatus, writeSnapshot } from './cache.ts';
import { fetchBatch, fetchRental, getTokenInfo } from './ura-client.ts';
import { parseDate, avg, med, safeDiv, sleep } from './helpers.ts';
import { Agg } from './aggregator.ts';
import { buildBedroomModel } from './bedroom.ts';
import {
...
import type { RentalAggData, DashboardData, CacheInfo, RentalRecord } from '../types.ts';

// ═══ FETCH & AGGREGATE REAL RENTAL DATA ═══

export async function fetchRentalData(): Promise<RentalAggData | null> {
...
export async function buildDashboardData(force: boolean = false): Promise<DashboardData> {
...
function _rebuildProjYearData() {
  ...
  for (const r of salesStore) {
    ...
    if (!projAgg[r.p][year]) projAgg[r.p][year] = { s: 0, n: 0, ps: 0 };
    projAgg[r.p][year].s += r.ps; projAgg[r.p][year].n++; projAgg[r.p][year].ps += r.pr;
    if (!projAreas[r.p]) projAreas[r.p] = [];
    if (projAreas[r.p].length < 200) projAreas[r.p].push(r.a);
  }
  ...
  const globalAvgArea = salesStore.length > 0 ? Math.round(salesStore.reduce((s, r) => s + r.a, 0) / salesStore.length) : 850;
  ...
  for (const [name, meta] of Object.entries(idx)) {
    ...
    if (projAgg[name]) {
      for (const [y, yData] of Object.entries(projAgg[name])) {
        ...
        yearPrice[y] = yData.ps > 0 ? Math.round(yData.ps / yData.n) : 0;
      }
    }
    const areas = projAreas[name] || [];
    const avgArea = areas.length > 0 ? Math.round(areas.reduce((s, a) => s + a, 0) / areas.length) : globalAvgArea;
    ...
    newProjYearData[name] = {
      ...
      n: meta.n, type: meta.type, psf: meta.psf, yield: yld, yearPsf, yearPrice, avgArea,
      hasRealRental: !!rp, rent,
    };
  }

  // Fix projIndex yield, yearPsf, yearPrice
  for (const [name, meta] of Object.entries(idx)) {
    const rp = rentalByProj[name];
    ...
    meta.yearPrice = newProjYearData[name]?.yearPrice || {};
    meta.avgArea = newProjYearData[name]?.avgArea || meta.avgArea || globalAvgArea;
  }
...
export function getFullCacheInfo(): Record<string, any> {
