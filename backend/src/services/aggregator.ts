...
interface ProjBucket { name: string; street: string; seg: string; dist: string; tenure: string; pType: string; s: number; n: number; areas: number[]; prices: number[]; byY: Record<string, { s: number; n: number }>; byFl: Record<string, { s: number; n: number }>; latest: string }
...
export class Agg {
  ...
  build(rentalData: RentalAggData | null): Record<string, any> {
    ...
    for (const p of Object.values(this.byProj)) {
      ...
      projIndex[p.name] = { dist: p.dist, seg: p.seg, psf: psfVal, n: p.n, yield: yld, street: p.street, type: p.pType, yearPsf, avgArea: pAvgArea };
      projYearDataLocal[p.name] = { street: p.street, dist: p.dist, seg: p.seg, n: p.n, type: p.pType, psf: psfVal, yield: yld, yearPsf, avgArea: pAvgArea };
    }
