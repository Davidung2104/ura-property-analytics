...
export class Agg {
  ...
  build(rentalData: RentalAggData | null): Record<string, any> {
    ...
    for (const p of Object.values(this.byProj)) {
      ...
      const yearPsf = {};
      for (const [y, yData] of Object.entries(p.byY)) { yearPsf[y] = avg(yData.s, yData.n); }
      projIndex[p.name] = { dist: p.dist, seg: p.seg, psf: psfVal, n: p.n, yield: yld, street: p.street, type: p.pType, yearPsf };
      projYearDataLocal[p.name] = { street: p.street, dist: p.dist, seg: p.seg, n: p.n, type: p.pType, psf: psfVal, yield: yld, yearPsf };
    }
